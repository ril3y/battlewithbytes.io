...
export default function SerialTerminal({
  ...
  const [sendOptions, setSendOptions] =
    useState<SendOptions>(DEFAULT_SEND_OPTIONS);
  ...
  useEffect(() => {
    if (terminalState.isConnected) {
      statsInterval.current = setInterval(() => {
        ...
        const rxRate = Math.round(
          (terminalState.bytesReceived / duration) * 1000,
        );
    ...
    terminalState.bytesReceived,
    terminalState.bytesSent,
  ]);
  ...
  const handleConnect = useCallback(async () => {
    if (!isSerialSupported()) {
      ...
      terminalRef.current?.writeln(
        "\x1b[31mError: Web Serial API not supported\x1b[0m",
      );
      return;
    }
    ...
    return (
      <div className="p-8 bg-red-900/20 border border-red-600/50 rounded-lg">
        ...
        <p className="text-red-300 mb-4">
          This tool requires the Web Serial API, which is not supported in your
          current browser.
        </p>
        <p className="text-red-300">
          Please use <strong>Chrome</strong>, <strong>Edge</strong>, or{" "}
          <strong>Opera</strong> to access this feature.
        </p>
      </div>
    );
  }
  ...
  return (
    <div
      className={`bg-gray-900/50 border-2 border-green-500/30 rounded-lg shadow-[0_0_20px_rgba(34,197,94,0.15)] flex flex-col ${isStandalone ? "h-full" : ""}`}
    >
      ...
      <div
        className={`bg-black ${isStandalone ? "flex-1 min-h-0" : ""}`}
