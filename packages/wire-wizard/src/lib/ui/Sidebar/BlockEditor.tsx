/**
 * Block Editor (Context-based)
 * Edit properties of selected block including connection points
 *
 * Uses contexts instead of props for:
 * - blocks, updateBlock, removeBlock
 * - selectedBlockId, selectedPointId
 * - addConnectionPoint, updateConnectionPoint, removeConnectionPoint
 */
...
interface BlockEditorProps {
  // Only props that can't come from context
...
export const BlockEditor: React.FC<BlockEditorProps> = ({
  ...
  // Local editing state for the currently selected point
  ...
  // Sync editingPoint when selection changes
  useEffect(() => {
    if (selectedBlockId && selectedPointId) {
      const block = blocks.find(b => b.id === selectedBlockId);
      const point = block?.connectionPoints.find(p => p.id === selectedPointId);
      if (point) {
  ...
  }, [selectedBlockId, selectedPointId, blocks]);

  // Handle point selection
  const handlePointClick = useCallback((point: ConnectionPoint) => {
    if (selectedBlockId) {
      selectPoint(selectedBlockId, point.id);
    }
  }, [selectedBlockId, selectPoint]);

  // Handle add connection point
  const handleAddConnectionPoint = useCallback(() => {
    if (selectedBlockId) {
      addConnectionPoint(selectedBlockId);
    }
  }, [selectedBlockId, addConnectionPoint]);

  // Handle remove connection point
  const handleRemoveConnectionPoint = useCallback((pointId: string) => {
    if (selectedBlockId) {
      removeConnectionPoint(selectedBlockId, pointId);
    }
  }, [selectedBlockId, removeConnectionPoint]);
  ...
  }, [selectedBlockId, removeBlock, clearSelection]);
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '10px' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px', color: '#fff', cursor: 'pointer' }}>
            <input
              type="checkbox"
              checked={selectedBlock.showConnectionLabels !== false}
      ...
      <div style={{ marginBottom: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
          ...
          <div style={{ display: 'flex', gap: '5px' }}>
            ...
            {onStartPlacement && onStopPlacement && (
              <button
                onClick={() => {
                  if (placementMode) {
                    onStopPlacement();
                  } else {
                    onStartPlacement(selectedBlock.id);
                  }
                }}
                style={{
