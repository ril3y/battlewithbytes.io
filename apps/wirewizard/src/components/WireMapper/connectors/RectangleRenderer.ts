...
function calculatePinNumber(
  ...
  if (pinLayout && pinLayout.length > 0 && pinLayout.length !== actualRows) {
    // Check if it's truly a custom pattern
    console.warn(
      "Pin numbering for custom patterns is currently sequential only.",
    );
    return pinIndex;
  }
...
export class RectangleRenderer implements IConnectorRenderer {
    ...
    const configOptions: ConfigOption[] = [
      {
        ...
        max: 20, // Example max
        defaultValue: 1,
        disabledCondition: (config: ConnectorConfig) =>
          !!(typeof config.pinPattern === "string" && config.pinPattern.trim()), // Disable if pinPattern has value
        required: true,
      ...
      },
      {
        ...
        required: true,
        description:
          "Number of columns per row, or max columns for Pin Pattern. Used if Pin Pattern is not set.",
      },
      {
        key: "pinPattern",
        ...
        description:
          "Define exact pin counts for each row, comma-separated (e.g., 8,6,6,7). Overrides Rows and Columns inputs for pin layout.",
      },
      {
  ...
  generatePins(
    ...
    const centerPinsHorizontally =
      typeof config.centerPinsHorizontally === "boolean"
        ? config.centerPinsHorizontally
        : true;
    const pins: Pin[] = [];
    ...
    console.log(`Starting generatePins with config:`, {
      rows,
      cols,
      pinPattern,
    });
    ...
    if (pinPattern && typeof pinPattern === "string" && pinPattern.trim()) {
      console.log(
        `Parsing pin pattern: "${pinPattern}" typeof=${typeof pinPattern}`,
      );

      // Simple split and parse approach for reliability
      const parts = pinPattern.trim().split(",");
      console.log(
        `  Split result (${parts.length} parts): "${parts.join('", "')}"`,
      );
      ...
      console.log(
        `  Pattern valid? ${validPattern}, parts: [${patternParts.join(",")}]`,
      );
      ...
      if (pinPattern === "3,3") {
        console.log(
          `  SPECIAL CASE: Pattern is exactly "3,3" which should create 2 rows with 3 pins each`,
        );
      }
      ...
      if (validPattern && patternParts.length > 0) {
        ...
        console.log(
          `  USING PIN PATTERN: [${pinLayout.join(",")}], rows=${actualRows}, totalPins=${totalPins}`,
        );
        console.log(`  Pins per row: ${JSON.stringify(pinLayout)}`);
      } else {
        console.warn(
          `  Invalid pinPattern format: "${pinPattern}". Falling back to rows/cols.`,
        );
      ...
      console.log(
        `No pinPattern, using standard grid: ${rows} rows x ${cols} columns`,
      );
      totalPins = rows * cols;
      pinLayout = Array(rows).fill(cols);
      actualRows = rows;
    ...
    for (let r = 0; r < pinLayout.length; r++) {
      ...
      for (let c = 0; c < pinsInThisRow; c++) {
        ...
        console.log(
          `  Placing pin ${globalPinIndex} at row=${currentRow}, col=${currentCol}`,
        );

        // Calculate pin number based on chosen numbering scheme
        const pinNumber = calculatePinNumber(
          globalPinIndex,
          config,
          actualRows,
          pinLayout,
        ...
        console.log(
          `  ✅ Created pin ${globalPinIndex} at R${currentRow}C${currentCol}, pos=(${xPos}, ${yPos})`,
        );
        pins.push(pin);
      }
  ...
  getPinPosition(
    ...
    if (
      pinLayoutParam &&
      pinLayoutParam.length > 0 &&
      pinLayoutParam.every((p) => p > 0)
    ) {
      pinLayout = pinLayoutParam;
    } else if (pinLayout.length === 0) {
      // No valid pattern or param, use rows/cols default
      pinLayout = Array(rows).fill(cols);
    }
