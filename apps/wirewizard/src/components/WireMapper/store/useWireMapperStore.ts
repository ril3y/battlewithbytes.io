...
interface WireMapperState {
  ...
  updateMapping: (
    id: string,
    updates: Partial<Omit<Mapping, "id" | "wireId">>,
  ) => void;
...
export const useWireMapperStore = create<WireMapperState>((set, get) => {
  ...
  const _disconnectPin = (
    wireIdToRemove: string,
    draft: WireMapperState,
  ): boolean => {
    console.log(
      `[_disconnectPin] Start: Removing connection for wireId ${wireIdToRemove}`,
    );

    // Find the mapping and wire to remove
    const mappingIndex = draft.mappings.findIndex(
      (m) => m.wireId === wireIdToRemove,
    );
    ...
    if (mappingIndex === -1) {
      console.warn(
        `[_disconnectPin] Mapping not found for wireId ${wireIdToRemove}.`,
      );
      // Attempt to remove the wire anyway if it exists, as it's now orphaned.
      if (wireIndex !== -1) {
        console.log(
          `[_disconnectPin] Removing orphaned wire ${wireIdToRemove}.`,
        );
    ...
    console.log(
      `[_disconnectPin] Removing mapping index ${mappingIndex} for wire ${wireIdToRemove}.`,
    );
    ...
    if (wireIndex !== -1) {
      console.log(
        `[_disconnectPin] Found wire index ${wireIndex} for wireId ${wireIdToRemove}. Removing wire.`,
      );
      draft.wires.splice(wireIndex, 1);
    } else {
      console.warn(
        `[_disconnectPin] Wire object ${wireIdToRemove} not found for removed mapping.`,
      );
    }
    ...
    pinsToUpdate.forEach((pinIdentifier) => {
      const connector = draft.connectors.find(
        (c) => c.id === pinIdentifier.connectorId,
      );
      ...
      if (pin) {
        ...
        if (index > -1) {
          ...
          console.log(
            `[_disconnectPin] Removed wire ${wireIdToRemove} from pin ${pinIdentifier.connectorId}-${pinIdentifier.pinPos}. Remaining: [${pin.connectedWireIds.join(", ")}]`,
          );
          ...
          if (pin.connectedWireIds.length === 0) {
            ...
            console.log(
              `[_disconnectPin] Pin ${pinIdentifier.connectorId}-${pinIdentifier.pinPos} has no connections left. Cleared netName and set color to undefined.`,
            );
          }
        } else {
          console.warn(
            `[_disconnectPin] Wire ${wireIdToRemove} not found in connectedWireIds of pin ${pinIdentifier.connectorId}-${pinIdentifier.pinPos}`,
          );
        }
      } else {
        console.warn(
          `[_disconnectPin] Pin ${pinIdentifier.connectorId}-${pinIdentifier.pinPos} not found while trying to remove wire ${wireIdToRemove}.`,
        );
      }
    });

    console.log(
      `[_disconnectPin] END: Finished removing connection for wireId ${wireIdToRemove}.`,
    );
  ...
  const _propagateNetUpdates = (
    ...
    console.log(
      `[_propagateNetUpdates] START: Propagating from ${startPinIdentifier.connectorId}-${startPinIdentifier.pinPos} with Color=${newColor}, NetName=${newNetName}`,
    );
    ...
    while (queue.length > 0) {
      ...
      if (visitedPins.has(visitedKey)) {
        console.log(
          `[_propagateNetUpdates] Pin ${visitedKey} already visited. Skipping.`,
        );
        continue;
      }
      ...
      const connector = draft.connectors.find(
        (c) => c.id === currentPinIdentifier.connectorId,
      );
      const pin = connector?.pins.find(
        (p) => p.pos === currentPinIdentifier.pinPos,
      );

      if (!pin) {
        console.warn(
          `[_propagateNetUpdates] Pin ${visitedKey} not found during propagation. Skipping.`,
        );
        continue;
      }
      ...
      console.log(
        `[_propagateNetUpdates] Updated pin ${visitedKey}: Color=${pin.config.color}, NetName=${pin.netName}`,
      );

      // If the pin has no connections, stop traversal for this branch
      if (!pin.connectedWireIds || pin.connectedWireIds.length === 0) {
        console.log(
          `[_propagateNetUpdates] Pin ${visitedKey} has no connections. Stopping traversal for this branch.`,
        );
        continue;
      }
      ...
      for (const wireId of pin.connectedWireIds) {
        console.log(
          `[_propagateNetUpdates] Processing connection via wire ${wireId} for pin ${visitedKey}`,
        );
        ...
        if (mapping) {
          console.log(
            `[_propagateNetUpdates] Found mapping ${mapping.id} for wire ${wireId}. Updating its color/netName.`,
          );
          ...
          if (wire) {
            console.log(
              `[_propagateNetUpdates] Found wire ${wire.id}. Updating its color.`,
            );
            const effectiveWireColor =
              newColor ?? get().settings.defaultWireColor ?? "#ffffff"; // Ensure string
            wire.color = effectiveWireColor; // Assign the guaranteed string color
          } else {
            console.warn(
              `[_propagateNetUpdates] Wire object not found for wireId ${wireId} (mapping ${mapping.id}) during propagation`,
            );
          }

          // Find the *other* pin connected by this mapping
          const otherPinIdentifier =
            mapping.source.connectorId === currentPinIdentifier.connectorId &&
            mapping.source.pinPos === currentPinIdentifier.pinPos
              ? mapping.target
              : mapping.source;
          ...
          if (!visitedPins.has(nextVisitedKey)) {
            console.log(
              `[_propagateNetUpdates] Queuing next pin ${nextVisitedKey} from mapping ${mapping.id}`,
            );
            queue.push(otherPinIdentifier);
          } else {
            console.log(
              `[_propagateNetUpdates] Next pin ${nextVisitedKey} already visited/queued.`,
            );
          }
        } else {
          console.warn(
            `[_propagateNetUpdates] Mapping object not found for pin ${visitedKey} (wireId: ${wireId}) during propagation`,
          );
        }
      }
    }
    console.log(
      `[_propagateNetUpdates] END: Finished propagation starting from ${startPinIdentifier.connectorId}-${startPinIdentifier.pinPos}`,
    );
  };
  ...
  return {
    ...
    loadProject: (project: WireMapperProject) =>
      set({
    ...
    // Settings actions
    updateSettings: (settings: Partial<WireMapperSettings>) =>
      set((state) => ({
    ...
    addConnector: (connectorData: Omit<Connector, "id">) => {
      ...
      set((state: WireMapperState) => {
        ...
        const pinPattern =
          typeof connectorData.config?.pinPattern === "string"
            ? connectorData.config.pinPattern.trim()
            : undefined;

        if (pinPattern) {
          try {
            const patternParts = pinPattern
              .split(",")
              .map((p: string) => parseInt(p.trim(), 10));
            if (
              patternParts.length > 0 &&
              patternParts.every((p: number) => !isNaN(p) && p > 0)
            ) {
              actualRows = patternParts.length;
              actualCols = Math.max(...patternParts);
              console.log(
                `[Store] Using pinPattern: ${pinPattern}. Calculated rows=${actualRows}, cols=${actualCols}`,
              );
            } else {
              console.warn(
                `[Store] Invalid pinPattern format: "${pinPattern}". Falling back to rows/cols.`,
              );
            }
          } catch (error) {
            console.error(
              `[Store] Error parsing pinPattern: "${pinPattern}"`,
              error,
            );
          }
        }
        ...
        const pins =
          connectorData.pins && connectorData.pins.length > 0
            ? connectorData.pins
    ...
    removeConnector: (id: string) =>
      set((state: WireMapperState) => {
        ...
        const updatedMappings = state.mappings.filter(
          (m) => m.source.connectorId !== id && m.target.connectorId !== id,
        );
        // Also remove wires connected to this connector
        const updatedWires = state.wires.filter(
          (w) => w.source.connectorId !== id && w.target.connectorId !== id,
        );
        return {
          ...
          selectedPin:
            state.selectedPin?.connectorId === id ? null : state.selectedPin,
        };
      }),

    duplicateConnector: (connectorId: string) =>
      set((state: WireMapperState) => {
        const originalConnector = state.connectors.find(
          (c) => c.id === connectorId,
        );
        if (!originalConnector) {
          console.error(
            `[Store] Connector not found for duplication: ${connectorId}`,
          );
        ...
        const newConnector: Connector = {
          ...
          config: originalConnector.config
            ? JSON.parse(JSON.stringify(originalConnector.config))
            : {},
          ...
          pins: originalConnector.pins.map((originalPin) => {
            return {
              ...
              config: originalPin.config
                ? JSON.parse(JSON.stringify(originalPin.config))
                : {}, // Deep clone config
              active: originalPin.active, // Copy active state

              // Copy optional properties if they exist
              ...(originalPin.number !== undefined && {
                number: originalPin.number,
              }),
              ...(originalPin.visible !== undefined && {
                visible: originalPin.visible,
              }),
              ...(originalPin.row !== undefined && { row: originalPin.row }),
              ...(originalPin.col !== undefined && { col: originalPin.col }),

              // Copy net-related properties
              ...(originalPin.netName !== undefined && {
                netName: originalPin.netName,
              }),
              ...(originalPin.netColor !== undefined && {
                netColor: originalPin.netColor,
              }),
              ...(originalPin.desc !== undefined && { desc: originalPin.desc }),
              ...(originalPin.voltage !== undefined && {
                voltage: originalPin.voltage,
              }),
              ...(originalPin.signalType !== undefined && {
                signalType: originalPin.signalType,
              }),
            };
          }),
        };
    ...
    setSelectedConnector: (connectorId: string | null) =>
      set(() => {
        return {
    ...
    updateConnectorPosition: (id: string, x: number, y: number) =>
      set((state: WireMapperState) => ({
        connectors: state.connectors.map((c: Connector) =>
          c.id === id
            ? {
    ...
    rotateConnector: (id: string, rotation: number) =>
      set((state: WireMapperState) => ({
        connectors: state.connectors.map((c: Connector) =>
    ...
    // Pin actions
    updatePin: (connectorId: string, pinPos: number, updates: Partial<Pin>) =>
      set((state: WireMapperState) => ({
    ...
    updatePinDetailsAndNet: (
      ...
      console.log(
        `[updatePinDetailsAndNet] START: Updating pin ${connectorId}-${pinPos} with`,
        updates,
      );
      set(
        produce((draft: WireMapperState) => {
          const connector = draft.connectors.find(
          ...
          if (!pin) {
            console.error(
              `Pin ${pinPos} on connector ${connectorId} not found`,
            );
            return;
          }
          ...
          if (updates.name !== undefined) {
            console.log(
              `[updatePinDetailsAndNet] Updating pin name to: ${updates.name}`,
            );
          ...
          let targetNetName =
            netNameProp !== undefined
              ? netNameProp
              : updates.netName !== undefined
                ? updates.netName
                : pin.netName;

          // --- Net Name Validation for UNCONNECTED pins ---
          let allowNetNameUpdate = true;
          if (pin.connectedWireIds.length === 0 && targetNetName) {
            // Only validate if unconnected and setting a name
            console.log(
              `[updatePinDetailsAndNet] Pin ${connectorId}-${pinPos} is unconnected. Validating target net name: ${targetNetName}`,
            );
            for (const otherConnector of draft.connectors) {
              for (const otherPin of otherConnector.pins) {
                // Skip self-comparison
                if (
                  otherConnector.id === connectorId &&
                  otherPin.pos === pinPos
                )
                  continue;
                ...
                if (
                  otherPin.connectedWireIds.length === 0 &&
                  otherPin.netName === targetNetName
                ) {
                  console.warn(
                    `[updatePinDetailsAndNet] Validation FAILED: Unconnected pin ${otherConnector.id}-${otherPin.pos} already has net name '${targetNetName}'. Cannot assign to ${connectorId}-${pinPos}.`,
                  );
          ...
          if (allowNetNameUpdate) {
            console.log(
              `[updatePinDetailsAndNet] Validation passed or not required. Updating net name for ${connectorId}-${pinPos} to: ${targetNetName}`,
            );
            pin.netName = targetNetName;
          } else {
            console.log(
              `[updatePinDetailsAndNet] Keeping original net name for ${connectorId}-${pinPos} due to validation failure.`,
            );
          ...
          if (pin.connectedWireIds.length) {
            ...
            console.log(
              `[updatePinDetailsAndNet] Pin is connected. Calling _propagateNetUpdates with Color=${newColor}, NetName=${netNameToPropagate}`,
            );
            _propagateNetUpdates(
              { connectorId, pinPos },
              newColor,
              netNameToPropagate,
              draft,
            ...
            console.log(
              `[updatePinDetailsAndNet] Pin is not connected. Skipping propagation.`,
            );
      ...
      console.log(
        `[updatePinDetailsAndNet] END: Finished update for pin ${connectorId}-${pinPos}`,
      );
    },

    setSelectedPin: (connectorId: string | null, pinPos: number | null) =>
      set({
        // Added types
        selectedPin:
          connectorId && pinPos !== null ? { connectorId, pinPos } : null,
    ...
    setCopiedNet: (netInfo: CopiedNetInfo | null) =>
      set({
        copiedNet: netInfo,
      }),
    ...
    copyNetFromPin: (connectorId: string, pinPos: number) => {
      ...
      const copiedNet: CopiedNetInfo = {
        netName: pin.netName,
        netColor:
          typeof pin.config?.color === "string" ? pin.config.color : undefined, // Ensure color is a string
        voltage:
          typeof pin.voltage === "string"
            ? pin.voltage
            : pin.voltage?.toString(), // Convert to string if it's a number
    ...
    // Mapping actions
    addMapping: (newMapping: Omit<Mapping, "id" | "wireId">) =>
      set(
        produce((draft: WireMapperState) => {
          console.log(
            `[addMapping] Start: Source=${newMapping.source.connectorId}-${newMapping.source.pinPos}, Target=${newMapping.target.connectorId}-${newMapping.target.pinPos}`,
          );
       ...
       */
          ...
          const sourcePinState = draft.connectors
            .find((c) => c.id === newMapping.source.connectorId)
            ?.pins.find((p) => p.pos === newMapping.source.pinPos);
          const sourceColor = sourcePinState?.config?.color;
          const sourceNetName = sourcePinState?.netName;

          const targetPinState = draft.connectors
            .find((c) => c.id === newMapping.target.connectorId)
            ?.pins.find((p) => p.pos === newMapping.target.pinPos);
          ...
          const authoritativeNetName =
            sourceNetName !== undefined ? sourceNetName : targetNetName; // Can be undefined
          console.log(
            `[addMapping] Determined authoritative Color: ${authoritativeColor}, NetName: ${authoritativeNetName}`,
          );
          ...
          const sourceConnector = draft.connectors.find(
            (c: Connector) => c.id === newMapping.source.connectorId,
          );
          const targetConnector = draft.connectors.find(
            (c: Connector) => c.id === newMapping.target.connectorId,
          );
          const sourcePin = sourceConnector?.pins.find(
            (p: Pin) => p.pos === newMapping.source.pinPos,
          );
          const targetPin = targetConnector?.pins.find(
            (p: Pin) => p.pos === newMapping.target.pinPos,
          );
          ...
          console.log(
            `[addMapping] Adding wireId ${newWireId} to connectedWireIds for Source Pin ${sourcePin.pos} and Target Pin ${targetPin.pos}`,
          );
          // Ensure the arrays exist before pushing
          if (!Array.isArray(sourcePin.connectedWireIds))
            sourcePin.connectedWireIds = [];
          if (!Array.isArray(targetPin.connectedWireIds))
            targetPin.connectedWireIds = [];
          ...
          console.log(
            `[addMapping] Creating wire object for mapping ${finalMapping.id}`,
          );
          const wire = _createWireFromMapping(
            finalMapping,
            sourcePin,
            targetPin,
          );
          ...
          console.log(
            `[addMapping] Calling _propagateNetUpdates starting from source pin ${newMapping.source.connectorId}-${newMapping.source.pinPos} with Color=${authoritativeColor}, NetName=${authoritativeNetName}`,
          );
          _propagateNetUpdates(
            newMapping.source,
            authoritativeColor,
            authoritativeNetName,
            draft,
          );
          // --- End Net Merging/Creation Logic ---
          console.log(
            `[addMapping] End: Successfully added mapping ${newWireId}`,
          );
        }),
      ),
    ...
    updateMapping: (
      ...
      set(
        produce((draft: WireMapperState) => {
          ...
          const wire = draft.wires.find(
            (w: Wire) => w.id === originalMapping.wireId,
          );
          if (wire) {
    ...
    removeMapping: (mappingId: string) =>
      set(
        produce((draft: WireMapperState) => {
          const mappingToRemove = draft.mappings.find(
            (m) => m.id === mappingId,
          );
          ...
          console.log(
            `[removeMapping] Removing mapping ${mappingId} (Wire ID: ${wireIdToRemove})`,
          );
