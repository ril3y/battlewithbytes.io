...
export const Connector: React.FC<ConnectorProps> = ({
    ...
    return mappings.some(
      (mapping: Mapping) =>
        (mapping.source.connectorId === connector.id &&
          mapping.source.pinPos === pinIndex) ||
        (mapping.target.connectorId === connector.id &&
          mapping.target.pinPos === pinIndex),
    );
  };
  ...
  const getPinStyle = (pinData: { pin: Pin }): CSSProperties => {
    ...
    const isPinSelected =
      selectedPin?.connectorId === connector.id &&
      selectedPin?.pinIndex === pin.index;
    ...
    const isConnectingStartPin =
      connectingPin?.connectorId === connector.id &&
      connectingPin?.pinIndex === pin.index;

    // Use the pin's specific color if available, otherwise default by gender
    const pinColor =
      pin.config.color ?? (connector.gender === "Male" ? "#111111" : "#333333");
    ...
    } else if (isConnectingStartPin) {
      // Highlight for connection start
  ...
  const totalWidth =
    (connector.config.columns ?? 0) * pinSpacing + PIN_MARGIN * 2;
  const totalHeight =
    (connector.config.rows ?? 0) * pinSpacing + PIN_MARGIN * 2;
    ...
    if (pinWrapper) {
      ...
      if (pinPosStr) {
        ...
        if (!isNaN(pinIndex)) {
          ...
          console.error(
            "[Connector] Invalid pin position found in data attribute",
            pinPosStr,
          );
        }
      } else {
        console.error(
          "[Connector] Pin wrapper clicked, but data-pin-pos attribute missing.",
        );
      }
  ...
  const genderClass = (() => {
    switch (connector.gender) {
      case "Male":
        return "connector-male";
      case "Female":
        return "connector-female";
      default:
  ...
  return (
    <motion.div
      ...
      drag
      dragConstraints={
        dragConstraintsRef.current ? dragConstraintsRef : undefined
      } // Pass ref only if current exists
      dragMomentum={false}
    ...
    >
      {nameAbove && (
        <div
          className="text-center text-sm text-gray-300 mb-1 select-none w-full"
      ...
      {!nameAbove && (
        <div
          className="text-center text-sm text-gray-300 mt-1 select-none w-full"
          style={{ width: totalWidth }}
        >
          {connector.name || connector.type}
        </div>
