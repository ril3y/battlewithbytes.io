...
const ConnectorNode: React.FC<NodeProps<Connector>> = ({
  data,
  id,
  selected,
}) => {
  ...
  const handlePinContextMenu = useCallback(
    (e: React.MouseEvent, pinPos: number) => {
      e.preventDefault();
      e.stopPropagation();
  ...
  const calculatedWidth = useMemo(() => {
    const gridWidth =
      cols * PIN_SIZE + Math.max(0, cols - 1) * (PIN_MARGIN * 2);
    return gridWidth + CONNECTOR_PADDING * 2;
  }, [cols]);

  const calculatedHeight = useMemo(() => {
    const gridHeight =
      rows * PIN_SIZE + Math.max(0, rows - 1) * (PIN_MARGIN * 2);
                ...
                {pinsInRow.map((pin) => (
                  <div key={`pin-${pin.pos}`} style={{ position: "relative" }}>
                    <PinDisplay
                      pin={pin}
                      isSelected={
                        selectedPin?.connectorId === id &&
                        selectedPin?.pinPos === pin.pos
                      }
          ...
          >
            {visiblePins.map((pin) => {
              if (pin.row === undefined || pin.col === undefined) {
                console.warn(
                  `Pin ${pin.pos} in connector ${id} is missing row/col data.`,
                );
                return null;
              }
