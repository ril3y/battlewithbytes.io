...
export const ConnectorDetail: React.FC<ConnectorDetailProps> = ({
  ...
  useEffect(() => {
    ...
    const hasConnections = connector.pins.some(
      (pin) => pin.connectedWireIds && pin.connectedWireIds.length > 0,
    );
    ...
    if (!localData || localData.id !== connector.id) {
      ...
      setLocalData({
        ...
        config: connector.config
          ? { ...JSON.parse(JSON.stringify(connector.config)) }
          : {},
      });
    } else {
      ...
      const fieldsToSync: (keyof Connector)[] = [
        "name",
        "type",
        "gender",
        "rows",
      ...
      fieldsToSync.forEach((field) => {
        if (
          ...
          return; // these are handled differently or not directly editable here
        ...
        if (String(normalizedStoreValue) !== String(normalizedLocalValue)) {
          // Compare as strings for simplicity here, refine if type issues arise
  ...
  if (!localData || !connector) {
    return (
      <p className="text-gray-500">No connector selected or data loaded.</p>
    );
  }

  const inputClass =
    "w-full p-2 bg-gray-800 border border-gray-700 rounded text-gray-300 focus:ring-1 focus:ring-green-500 focus:border-green-500";
  ...
  return (
    <div className="space-y-4">
      <div>
        <label htmlFor="connectorName" className={labelClass}>
          Name
        </label>
        <input
          type="text"
      ...
      <div>
        <label htmlFor="connectorType" className={labelClass}>
          Type (e.g., DB9, JST-XH)
        </label>
        <input
          type="text"
      ...
      <div>
        <label htmlFor="connectorGender" className={labelClass}>
          Gender
        </label>
        <select
          id="connectorGender"
      ...
      <div className="border-t border-gray-700 pt-4">
        <h4 className="text-md font-semibold text-green-500 mb-2">
          Layout Configuration
        </h4>
        {isLayoutLocked && (
          <p className="text-xs text-yellow-400 bg-yellow-900/50 p-2 rounded mb-3">
            Layout properties (rows, columns, etc.) are locked because this
            connector has active wire connections. To modify layout, please
            remove all wires first or use the &apos;Edit in Builder&apos; option
            which may re-generate pins.
          </p>
        )}
        <div>
          <label htmlFor="connectorRows" className={labelClass}>
            Rows
          </label>
          <input
            type="number"
        ...
        <div>
          <label htmlFor="connectorCols" className={labelClass}>
            Columns
          </label>
          <input
            type="number"
        ...
        {localData.config &&
          typeof localData.config.centerPinsHorizontal === "boolean" && (
            <div>
              <label
                htmlFor="centerPinsHorizontal"
                className={`${labelClass} mt-2 flex items-center`}
              >
                <input
                  type="checkbox"
