  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          height: `${CONNECTOR_HEADER_HEIGHT}px`,
          display: "flex",
          alignItems: "center",
      ...
      <div
        ...
        style={{
          height: `${CONNECTOR_TYPE_HEADER_HEIGHT}px`,
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
        }}
            ...
            if (position === "left") {
              return (
                <tr
                  ...
                  style={{ height: `${PIN_ROW_HEIGHT}px` }}
                >
                  ...
                  <td
                    className="py-1 px-2 text-center text-gray-400 text-xs"
                    style={{ width: "35%" }}
                  >
                    {displayNet}
                  </td>
              ...
              return (
                ...
                >
                  ...
                  <td
                    className="py-1 px-2 text-right text-gray-400 text-xs"
                    style={{ width: "35%" }}
                  >
                    {displayNet}
                  </td>
...
export const WiringDiagramPreview: React.FC = () => {
  ...
  const {
    ...
    mappings.forEach((mapping) => {
      connectionCount.set(
        ...
        (connectionCount.get(mapping.source.connectorId) || 0) + 1,
      );
      connectionCount.set(
        mapping.target.connectorId,
        (connectionCount.get(mapping.target.connectorId) || 0) + 1,
      );
      usedPinIdsTemp.add(
        `${mapping.source.connectorId}-${mapping.source.pinPos}`,
      );
      usedPinIdsTemp.add(
        `${mapping.target.connectorId}-${mapping.target.pinPos}`,
      );

      const sourceConnFull = connectors.find(
        (c) => c.id === mapping.source.connectorId,
      );
      const targetConnFull = connectors.find(
        (c) => c.id === mapping.target.connectorId,
      );

      if (sourceConnFull && targetConnFull) {
        const sourcePinFull = sourceConnFull.pins.find(
          (p) => p.pos === mapping.source.pinPos,
        );
        const targetPinFull = targetConnFull.pins.find(
          (p) => p.pos === mapping.target.pinPos,
        );
        ...
        if (sourcePinFull && targetPinFull) {
          ...
          wireColorsMap.set(
            `${sourceConnFull.id}-${sourcePinFull.pos}`,
            chosenWireColor,
          );
          wireColorsMap.set(
            `${targetConnFull.id}-${targetPinFull.pos}`,
            chosenWireColor,
          );
    ...
    const getPins = (connector: Connector) =>
      connector.pins
    ...
    const processedWires = mappings.reduce((acc, mapping) => {
      const sourceConn = connectors.find(
        (c) => c.id === mapping.source.connectorId,
      );
      const targetConn = connectors.find(
        (c) => c.id === mapping.target.connectorId,
      );
      if (!sourceConn || !targetConn) return acc;

      const sourcePin = sourceConn.pins.find(
        (p) => p.pos === mapping.source.pinPos,
      );
      const targetPin = targetConn.pins.find(
        (p) => p.pos === mapping.target.pinPos,
      );
      ...
      if (sourceIsInLeftArray && targetIsInRightArray) {
        ...
        trueLeftConnector = sourceConn;
        trueLeftPin = sourcePin;
        trueRightConnector = targetConn;
        trueRightPin = targetPin;
        ...
        trueLeftConnector = targetConn;
        trueLeftPin = targetPin;
        trueRightConnector = sourceConn;
        trueRightPin = sourcePin;
  ...
  const { connectorPositions, diagramHeight, diagramWidth } = useMemo(() => {
    ...
    const calculateConnectorHeight = (connector: Connector) => {
      const numPins = connector.pins.filter((p) =>
        usedPinIds.has(`${connector.id}-${p.pos}`),
      ).length;
      return (
        CONNECTOR_HEADER_HEIGHT +
        CONNECTOR_TYPE_HEADER_HEIGHT +
        numPins * PIN_ROW_HEIGHT
      );
    };
    ...
    const calculatedDiagramHeight =
      Math.max(currentLeftY, currentRightY) -
      VERTICAL_GAP_BETWEEN_CONNECTORS +
      SVG_PADDING;
    const calculatedDiagramWidth =
      SVG_PADDING * 2 + CONNECTOR_WIDTH * 2 + HORIZONTAL_GAP_BETWEEN_COLUMNS;
    ...
    rightConnectorsWithPins.forEach((connector) => {
      ...
      if (pos) {
        positions.set(connector.id, {
          ...pos,
          x: calculatedDiagramWidth - CONNECTOR_WIDTH - SVG_PADDING,
        });
      }
    });
  ...
  const renderWires = () => {
    ...
    return wireInfosProcessed.map((wire) => {
      ...
      const sourceConnList =
        wire.isSameColumn && wire.columnSide === "left"
          ? leftConnectorsWithPins
          : wire.isSameColumn && wire.columnSide === "right"
            ? rightConnectorsWithPins
            : leftConnectorsWithPins;
      const targetConnList =
        wire.isSameColumn && wire.columnSide === "left"
          ? leftConnectorsWithPins
          : wire.isSameColumn && wire.columnSide === "right"
            ? rightConnectorsWithPins
      ...
      const leftPinVisualIndex =
        sourceConnList
          .find((c) => c.id === wire.leftConnector.id)
          ?.pins.findIndex((p) => p.pos === wire.leftPin.pos) ?? -1;

      const rightPinVisualIndex =
        targetConnList
          .find((c) => c.id === wire.rightConnector.id)
          ?.pins.findIndex((p) => p.pos === wire.rightPin.pos) ?? -1;
      ...
      if (wire.isSameColumn) {
        ...
        const connX =
          wire.columnSide === "left"
            ? leftConnPos.x
            : rightConnPos.x + CONNECTOR_WIDTH;
      ...
      let curveStrength = Math.min(
        yDiff / 1.5,
        HORIZONTAL_GAP_BETWEEN_COLUMNS / 1.5,
      ); // Increased curve strength
  ...
  return (
    <div
      ref={diagramRef}
      className="relative w-full h-full overflow-auto bg-gray-900 p-4"
      data-wiring-diagram
    >
      <div
        style={{
