...
export const EditConnector: React.FC<EditConnectorProps> = ({
  connectorId,
  onComplete,
  onCancel,
}) => {
  ...
  const [pinRemovalMode, setPinRemovalMode] = useState<"skip" | "reindex">(
    "skip",
  );
  ...
  useEffect(() => {
    if (connector) {
      ...
      const initialPinLayout = Array.from({
        length: rowsValue * colsValue,
      }).map((_, index) => {
        ...
        return {
          ...
          col,
          visible:
            existingPin?.visible !== undefined ? existingPin.visible : true, // Default to visible
      ...
      const defaultCounts = counts.map((count) =>
        count === 0 ? colsValue : count,
      );
      ...
      const newPinLayout = Array.from({ length: rows * cols }).map(
        (_, index) => ({
          pos: index + 1,
  ...
  const handleRowsChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(newRows) && newRows > 0) {
      ...
      if (editMode !== "advanced") {
        const newLayout = Array.from({ length: newRows * cols }).map(
          (_, index) => {
            const pos = index + 1;
  ...
  const handleColsChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(newCols) && newCols > 0) {
      ...
      if (editMode !== "advanced") {
        const newLayout = Array.from({ length: rows * newCols }).map(
          (_, index) => {
            const pos = index + 1;
            const existingPin = pinLayout.find((p) => p.pos === pos);
            return {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
      <div className="bg-gray-900 p-6 rounded-lg w-full max-w-md">
        <h2 className="text-xl text-green-400 font-mono mb-4">
          Edit Connector
        </h2>
            ...
            <div className="mt-2 p-3 bg-gray-850 rounded border border-gray-700">
              ...
              <div className="mt-4">
                ...
                >
                  {pinLayout.map((pin) => {
                    ...
                    if (pin.visible && pinRemovalMode === "reindex") {
                      ...
                      const reindexPos = visiblePins.findIndex(
                        (p) => p.pos === pin.pos,
                      );
                      if (reindexPos !== -1) {
                ...
                <div className="text-xs text-gray-500 mt-1">
                  Active pins: {pinLayout.filter((p) => p.visible).length} of{" "}
                  {rows * cols}
                </div>
                <button
                  type="button" // Prevent form submission
