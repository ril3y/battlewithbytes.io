...
export const MappingList: React.FC<MappingListProps> = ({
  ...
  const filteredMappings = filterConnectorId
    ? mappings.filter(
        (m) =>
          m.source.connectorId === filterConnectorId ||
          m.target.connectorId === filterConnectorId,
      )
      ...
      <div className="flex-grow overflow-y-auto min-h-0">
        <table className="w-full text-sm text-left text-gray-300">
          <caption className="sr-only">
            Wire Mappings - Click a row to highlight that connection
          </caption>
            ...
            {filteredMappings.map((mapping) => (
              ...
              >
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <div className="font-mono text-xs">
                        {getConnectorName(mapping.source.connectorId)}
                      </div>
                ...
                <td className="px-3 py-2">
                  <div>
                    <div className="font-mono text-xs">
                      {getConnectorName(mapping.target.connectorId)}
                    </div>
                    <div className="text-gray-500 text-xs">
                      {getPinLabel(
                ...
                <td className="px-3 py-2">
                  <input
                    ...
                    onChange={(e) =>
                      updateMapping(mapping.id, { netName: e.target.value })
                    }
                ...
                <td className="px-3 py-2">
                  <select
                    value={mapping.gauge || ""}
                    onChange={(e) =>
                      updateMapping(mapping.id, { gauge: e.target.value })
                    }
                    className="w-full bg-gray-800 border border-gray-700 text-white rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-green-400"
                  >
                    <option value="">--</option>
