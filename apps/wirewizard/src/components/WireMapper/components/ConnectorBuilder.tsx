...
const getDefaultConfigFromSchema = (
  ...
  if (!("columns" in defaultConfig))
    defaultConfig.columns = CONNECTOR_DEFAULTS.cols;
  if (!("numPins" in defaultConfig))
    defaultConfig.numPins = CONNECTOR_DEFAULTS.numPins;
...
export const ConnectorBuilder: React.FC<ConnectorBuilderProps> = ({
  ...
  const [connectorState, setConnectorState] = useState<
    Omit<Connector, "id" | "pins"> & { id?: string; pins?: Pin[] }
  >(() => {
  ...
  const updateConnectorState = useCallback(
    <K extends keyof Connector>(
      updates:
        | Pick<Connector, K>
        | ((
            prev: Omit<Connector, "id" | "pins"> & {
              id?: string;
              pins?: Pin[];
            },
          ) => Pick<Connector, K>),
  ...
  useEffect(() => {
    ...
    setConnectorState((prev) => {
      ...
      if (newSchema.rows && !("rows" in updatedConfig))
        updatedConfig.rows = newDefaultConfig.rows;
      if (newSchema.columns && !("columns" in updatedConfig))
        updatedConfig.columns = newDefaultConfig.columns;
      if (newSchema.numPins && !("numPins" in updatedConfig))
        updatedConfig.numPins = newDefaultConfig.numPins;
  ...
  useEffect(() => {
    ...
    try {
      ...
      setPins((currentPins) => {
        const currentPinMapByIndex = new Map(
          currentPins.map((p) => [p.index, p]),
        );
    ...
    connectorState.shape,
    connectorState.width,
    connectorState.height,
  ]);

  const handleStateChange = useCallback(
    <K extends keyof Connector>(key: K, value: Connector[K]) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && connectorToEdit?.id) {
        updateConnector(
          connectorToEdit.id,
          finalConnector as Partial<Connector>,
        );
      } else {
      ...
      alert(
        `Failed to save connector: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
  ...
  return (
    <form
      ...
      className="space-y-6 p-6 bg-gray-800 rounded-lg shadow-lg max-w-2xl w-full mx-auto"
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              handleStateChange("gender", e.target.value as ConnectorGender)
            }
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              handleStateChange("shape", e.target.value as ConnectorShape)
            }
            className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent appearance-none"
          >
        ...
        <div className="space-y-4 md:col-span-2">
          ...
          {configSchema && Object.keys(configSchema).length > 0 ? (
            ...
            <p className="text-gray-500 italic">
              Select a shape to see configuration options.
            </p>
          )}
        </div>
