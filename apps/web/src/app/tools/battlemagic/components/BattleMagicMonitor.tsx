...
export default function BattleMagicMonitor() {
  ...
  const [selectedMemoryMapCpu, setSelectedMemoryMapCpu] =
    useState<string>("generic-cortex-m4");
  const [customMemoryRegions, setCustomMemoryRegions] = useState<
    MemoryRegion[]
  >([]);
        ...
        if (proj.firmware) {
          try {
            ...
            firmwareContext.setFirmwareData(
              firmwareData,
              proj.firmware.baseAddress,
            );
            gdb.addGdbOutput(
              `[Project] Restored firmware: ${firmwareData.length} bytes at 0x${proj.firmware.baseAddress.toString(16).toUpperCase()}`,
            );
          } catch (firmwareError) {
            console.error(
              "[BattleMagic] Failed to restore cached firmware:",
              firmwareError,
            );
            gdb.addGdbOutput(
              "[Warning: Failed to restore cached firmware from project]",
            );
          }
        }
      }
  ...
  useEffect(() => {
        ...
        try {
          ...
          if (reply.signal === 5 && pc !== undefined) {
            ...
            const normalizeAddress = (addr: string | number): string => {
              const addrStr =
                typeof addr === "number" ? addr.toString(16) : addr;
            ...
            console.log(
              "[Breakpoint Detection] PC:",
              `0x${pcHex}`,
              "Normalized:",
              pcNormalized,
            );
            console.log(
              "[Breakpoint Detection] Breakpoints in state:",
            ...
            const hitBreakpoint = debug.breakpoints.find(
              (bp) =>
            ...
            if (hitBreakpoint) {
              ...
              gdb.addGdbOutput(
                `[Target stopped] Signal: ${reply.signal} (PC: 0x${pcHex})`,
              );
              if (debug.breakpoints.length > 0) {
                gdb.addGdbOutput(
                  `[Debug] Active breakpoints: ${debug.breakpoints
            ...
            const pcInfo =
              pc !== undefined
                ...
                : "";
            gdb.addGdbOutput(
              `[Target stopped] Signal: ${reply.signal}${pcInfo}`,
            );
          }
    ...
    return () => {
      ...
      if (client.isConnected()) {
        console.log(
          "[BattleMagicMonitor] Cleaning up connection on unmount/reload",
        );
        client.disconnect().catch((error) => {
          console.error(
            "[BattleMagicMonitor] Cleanup disconnect error:",
            error,
          );
        });
      }
  ...
  const handleReadMemory = useCallback(
    async (address: number, length: number): Promise<Uint8Array | null> => {
      ...
      if (firmwareContext) {
        ...
        if (cachedData) {
          console.log(
            `[Monitor] Reading from firmware cache: 0x${address.toString(16)} (${length} bytes)`,
          );
          return cachedData;
        }
      }
  ...
  const handleToggleBreakpoint = useCallback(
    async (address: number) => {
      ...
      if (debug.executionState === "running") {
        gdb.addGdbOutput(
          "[Warning] Target is running - halt it first before setting debug.breakpoints",
        );
        gdb.addGdbOutput(
          "[Tip] Click the Pause (⏸) button to halt the target",
        );
        return;
      }
      ...
      try {
        if (existingBp) {
          ...
          debug.setBreakpoints(
            debug.breakpoints.filter((bp) => bp.id !== existingBp.id),
          );
          ...
          const currentHwBpCount = debug.breakpoints.filter(
            (bp) => bp.type === "hardware",
          ).length;
          if (currentHwBpCount >= 4) {
            gdb.addGdbOutput(
              `[⚠️ Warning] Already have ${currentHwBpCount} hardware debug.breakpoints set`,
            );
            gdb.addGdbOutput(
              `[⚠️ Warning] ARM Cortex-M typically supports 4-6 hardware debug.breakpoints max`,
            );
          }

          gdb.addGdbOutput(
            `[Debug] Attempting to set hardware breakpoint at ${addressStr}...`,
          );
          gdb.addGdbOutput(
            `[Debug] Current hardware breakpoints: ${currentHwBpCount}/~6`,
          );
          ...
          try {
            ...
            gdb.addGdbOutput(
              `[Debug] GDB insertBreakpoint threw error: ${insertError}`,
            );
            throw insertError;
          }
          ...
          debug.setBreakpoints([...debug.breakpoints, newBreakpoint]);
          gdb.addGdbOutput(
            `[✓ Breakpoint set at ${addressStr}] (HW #${currentHwBpCount + 1})`,
          );
        ...
        if (errorMsg.includes("empty response")) {
          gdb.addGdbOutput(
            `[❌ Failed to ${existingBp ? "remove" : "set"} breakpoint]`,
          );
          gdb.addGdbOutput(
            `   Hardware breakpoint command not supported by target`,
          );
        } else if (
          errorMsg.includes("E01") ||
          errorMsg.toLowerCase().includes("no more")
        ) {
          gdb.addGdbOutput(
            `[❌ Failed to set breakpoint - out of hardware breakpoint units]`,
          );
          gdb.addGdbOutput(
            `   ARM Cortex-M has limited hardware debug.breakpoints (typically 4-6)`,
          );
          gdb.addGdbOutput(
            `   Current debug.breakpoints set: ${debug.breakpoints.length}`,
          );
          gdb.addGdbOutput(
            `   Tip: Remove unused debug.breakpoints to free up hardware units`,
          );
        } else {
          gdb.addGdbOutput(
            `[❌ Failed to ${existingBp ? "remove" : "set"} breakpoint: ${errorMsg}]`,
          );
        }
  ...
  const handleClearAllBreakpoints = useCallback(async () => {
    ...
    // Show summary
    gdb.addGdbOutput(
      `[Breakpoint] Cleared: ${removedCount} removed from GDB, ${failedCount} were ghost debug.breakpoints`,
    );
    gdb.addGdbOutput(
      `[Breakpoint] UI state cleared: ${totalCount} breakpoint(s)`,
    );
  }, [gdb, debug, project]);
  ...
  const updateProjectState = useCallback(() => {
    ...
    uart.baudRate,
    selectedMemoryMapCpu,
    customMemoryRegions,
    debug.breakpoints,
    panels.activeRightPanel,
    project,
  ]);
  ...
  useEffect(() => {
    if (
      gdb.gdbState === ConnectionState.ATTACHED &&
    ...
    const autoLoadAnalysis = async () => {
      ...
      if (
        ...
        (stateDetails.state === "current" ||
          stateManager.shouldAutoLoad(currentProject))
      ) {
        try {
        ...
        gdb.addGdbOutput(
          `[Startup] Analysis available: ${stateDetails.statusMessage}`,
        );
        gdb.addGdbOutput(
          "[Startup] Go to Tools → Firmware Dump to load or re-analyze",
        );
      }
    };
  ...
  const gdbConnected =
    gdb.gdbState === ConnectionState.CONNECTED ||
    gdb.gdbState === ConnectionState.ATTACHED;
        ...
        <div className="battlemagic-container flex flex-col h-screen bg-gray-950 text-white overflow-hidden">
          ...
          <div className="bg-gray-900 border-b border-gray-700 px-4 py-2 flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-3">
              <Image
                src="/tools/battlemagic/battlemagiclogo.png"
              ...
              <>
                ...
                <div className="flex-1 flex overflow-hidden">
                  ...
                  <div
                    ...
                    style={{
                      width: panels.visiblePanels.console
                        ? `${100 - panels.consoleWidth}%`
                        : "100%",
                    }}
                  >
                    {/* Show DebuggerView by default when target is attached */}
                    {targetAttached &&
                      panels.activeRightPanel === "debugger" && (
                        <DebuggerView
                    ...
                    {panels.activeRightPanel === "target" && (
                      <TargetInfoPanel client={gdb.gdbClient} />
                    )}
                    ...
                    {panels.activeRightPanel === "memorymap" && (
                      <MemoryMapView
                        ...
                        onRegionSelect={(region) => {
                          gdb.addGdbOutput(
                            `[Selected memory region: ${region.name}]`,
                          );
                        }}
                      />
                    )}
                  ...
                  />
                    ...
                    {gdb.gdbClient && (
                      <div
                        style={{
                          display:
                            panels.activeRightPanel === "firmware-dump"
                              ? "block"
                              : "none",
                        }}
                      >
                        <FirmwareDumpWorkflow
                          ...
                          onOutput={gdb.addGdbOutput}
                          detectedArchInfo={
                            analysis.detectedArchInfo || undefined
                          }
                          onFirmwareCached={(cachedFirmware) => {
                            // Save firmware to project when dumped
                            if (project.projectManager) {
                              project.projectManager.updateFirmware(
                                cachedFirmware,
                              );
                              gdb.addGdbOutput(
                                "[Project] Firmware saved to project file",
                              );
                            }
                          }}
                          onAnalysisComplete={() => {
                            ...
                            panels.setActiveRightPanel("debugger");
                            gdb.addGdbOutput(
                              "[Analysis] 🎯 Switching to debugger view and jumping to PC...",
                            );
                  ...
                  {panels.visiblePanels.console && (
                    <>
                      ...
                      <div
                        className="h-full overflow-hidden flex flex-col bg-black"
                        style={{ width: `${panels.consoleWidth}%` }}
                      >
                          ...
                          <GdbPanel
                            ...
                            onAttachTarget={async (targetId) => {
                              ...
                              try {
                                ...
                                gdb.addGdbOutput(
                                  `[Attached to target ${targetId}]`,
                                );
                                const target = gdb.targets.find(
                                  (t) => t.id === targetId,
                                );
                                ...
                                if (target) {
                                  try {
                                    const archInfo = await detectArchitecture(
                                      target.description,
                                    );
                                    ...
                                    gdb.addGdbOutput(
                                      `[Analysis Support: ${archInfo.supported ? "✅ Supported" : "❌ Not Supported"}]`,
                                    );
                                    gdb.addGdbOutput(
                                      `[Match Confidence: ${(archInfo.confidence * 100).toFixed(1)}%]`,
                                    );
                                    ...
                                    if (archInfo.supported && analysisContext) {
                                      const analysisDb = getAnalysisDatabase();
                                      const hasMdbData =
                                        await analysisDb.hasAnalysis();
                                      const hasCachedFirmware =
                                        project.projectManager?.getCachedFirmware() !==
                                        undefined;
                                      ...
                                      analysis.setExistingAnalysisDetected(
                                        hasMdbData && hasCachedFirmware,
                                      );
                                      analysis.setShowAnalysisPrompt(true);
                                    }
                                  } catch (error) {
                                    gdb.addGdbOutput(
                                      `[Architecture detection failed: ${error}]`,
                                    );
                                  }
                                }
                ...
                <div className="space-y-3 mb-4 font-mono text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="text-green-400">
                      {analysis.detectedArchInfo.architecture}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">Manufacturer:</span>
                    <span className="text-white">
                      {analysis.detectedArchInfo.manufacturer}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">Chip:</span>
                    <span className="text-white">
                      {analysis.detectedArchInfo.chip_name}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">Confidence:</span>
                    <span className="text-green-400">
                      {(analysis.detectedArchInfo.confidence * 100).toFixed(1)}%
                    </span>
                  </div>
                </div>
                ...
                {analysis.existingAnalysisDetected ? (
                  <>
                    ...
                    <p className="text-gray-300 mb-6 text-sm">
                      You can load the existing analysis (fast) or re-analyze
                      the firmware (will reset chip to capture at interrupt
                      vector).
                    </p>
                    <div className="flex flex-col gap-3">
                        ...
                        onClick={async () => {
                          ...
                          if (
                            ...
                            try {
                              ...
                              analysis.setLoadingProgress({
                                stage: "Loading analysis from database...",
                                progress: 10,
                              });
                              gdb.addGdbOutput(
                              ...
                              analysis.setLoadingProgress({
                                stage:
                                  "Loading functions, xrefs, and comments...",
                                progress: 30,
                              });
                              const loaded =
                                await analysisContext.loadFromDatabase();
                              if (!loaded) {
                                analysis.setShowLoadingModal(false);
                                gdb.addGdbOutput(
                                  "[No analysis data found in database]",
                                );
                                return;
                              }
                              ...
                              analysis.setLoadingProgress({
                                stage: "Restoring firmware binary...",
                                progress: 60,
                              });
                              ...
                              if (cachedFirmware) {
                                try {
                                  ...
                                  firmwareContext.setFirmwareData(
                                    firmwareData,
                                    cachedFirmware.baseAddress,
                                  );
                                  gdb.addGdbOutput(
                                    `[Restored cached firmware: ${firmwareData.length} bytes at 0x${cachedFirmware.baseAddress.toString(16).toUpperCase()}]`,
                                  );
                                } catch (firmwareError) {
                                  console.error(
                                    "[BattleMagic] Failed to restore cached firmware:",
                                  ...
                                  gdb.addGdbOutput(
                                    "[Warning: Failed to restore cached firmware, disassembly may not work]",
                                  );
                                }
                              } else {
                                gdb.addGdbOutput(
                                  "[Warning: No cached firmware found, disassembly may not work]",
                                );
                              }

                              // Set PC to reset vector (entry point)
                              const vectorTable =
                                analysisContext.getVectorTable();
                              const resetVector = vectorTable.find(
                                (entry) => entry.vector_number === 1,
                              );
                              if (resetVector && resetVector.handler_address) {
                                debug.setProgramCounter(
                                  resetVector.handler_address,
                                );
                                gdb.addGdbOutput(
                                  `[PC set to reset vector: 0x${resetVector.handler_address.toString(16).toUpperCase()}]`,
                                );
                                ...
                                if (
                                  resetHandlerFunc &&
                                  resetHandlerFunc.name.startsWith("sub_")
                                ) {
                                  analysisContext.renameFunction(
                                    resetVector.handler_address,
                                    "entrypoint",
                                  );
                                  gdb.addGdbOutput(
                                    '[Renamed reset handler to "entrypoint"]',
                                  );
                                }
                              } else {
                                gdb.addGdbOutput(
                                  "[Warning: Reset vector not found in vector table]",
                                );
                              }
                              ...
                              gdb.addGdbOutput(
                                "[Switch to Debugger view to see disassembly]",
                              );
                      ...
                      <button
                        onClick={() => {
                          ...
                          gdb.addGdbOutput(
                            "[Re-analyzing] Will reset chip to capture firmware at interrupt vector",
                          );
                          gdb.addGdbOutput(
                            "[Re-analyzing] Starting firmware dump & analysis...",
                          );
                        }}
                  ...
                  <>
                    <div className="bg-amber-900/30 border border-amber-600 rounded p-3 mb-4">
                      <p className="text-amber-400 text-sm font-mono">
                        ⚠️ Will reset chip to capture firmware at interrupt
                        vector
                      </p>
                    </div>
                    <p className="text-gray-300 mb-6 text-sm">
                      This chip is supported for firmware analysis. The chip
                      will be reset to capture the firmware starting at the
                    ...
                    <div className="flex gap-3">
                      <button
                        onClick={() => {
                          ...
                          analysis.setShouldAutoStartAnalysis(true);
                          gdb.addGdbOutput(
                            "[Firmware Dump] Will reset chip to capture at interrupt vector",
                          );
                          gdb.addGdbOutput(
                            "[Firmware Dump] Starting dump & analysis...",
                          );
                        }}
                      ...
                      <button
                        onClick={() => {
                          analysis.setShowAnalysisPrompt(false);
                          gdb.addGdbOutput(
                            "[Analysis prompt dismissed - you can manually analyze later via Tools → Firmware Dump]",
                          );
                        }}
