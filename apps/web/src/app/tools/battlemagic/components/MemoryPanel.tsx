...
export default function MemoryPanel({ onReadMemory, isConnected, gdbClient, onOutput }: MemoryPanelProps) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.key === 'g' || e.key === 'G') && isMouseOverPanel && !showGoToModal && activeTab === 'memory') {
        e.preventDefault();
    ...
    try {
      ...
      if (isConnected) {
        try {
          ...
          setError(readErr instanceof Error ? readErr.message : 'Failed to read memory');
        } finally {
          setLoading(false);
        }
      } else {
      ...
      {showGoToModal && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50">
          <div className="bg-gray-900 border-2 border-green-500 rounded-lg shadow-2xl p-4 w-96">
            <h3 className="text-sm font-bold text-green-400 mb-3">Go To Memory Address</h3>
