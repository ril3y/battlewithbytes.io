...
export default function DebugControlToolbar({
  ...
  const executeCommand = useCallback(
    async (command: () => Promise<void>, commandName: string) => {
  ...
  const getButtonStyles = (button: ButtonConfig, isDisabled: boolean) => {
    const baseStyles =
      "px-3 py-2 text-sm rounded transition-all flex items-center gap-1 font-mono text-xs hover:shadow-lg";
    ...
    const variants = {
      ...
      success:
        "bg-green-600/30 border border-green-500/50 text-green-300 hover:bg-green-600/50 hover:border-green-400",
      warning:
        "bg-yellow-600/30 border border-yellow-500/50 text-yellow-300 hover:bg-yellow-600/50 hover:border-yellow-400",
      danger:
        "bg-red-600/30 border border-red-500/50 text-red-300 hover:bg-red-600/50 hover:border-red-400",
    };
  ...
  return (
    <div className="bg-gray-900 border-b border-gray-700 p-2 flex gap-1 flex-wrap flex-shrink-0">
      ...
      <div
        className={`flex items-center gap-2 px-2 pr-3 rounded transition-all ${
      ...
      >
        ...
        >
          ...
          <span
            className={
              executionState === ExecutionState.RUNNING ? "animate-pulse" : ""
            }
          >
      ...
      <div className="flex gap-1">
        {buttons.map((button) => {
          const isDisabled =
            button.disabled(executionState, isAttached) ||
            !isAttached ||
            isExecuting;
