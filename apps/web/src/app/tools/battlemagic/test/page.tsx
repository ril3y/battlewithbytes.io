...
export default function BattleMagicTestPage() {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setElfFile(file);
      addOutput(`Selected file: ${file.name} (${(file.size / 1024).toFixed(2)} KB)`);
    }
  };
  ...
  const detectMode = (bytes: Uint8Array): 'arm' | 'thumb' => {
    ...
    const e_flags = new DataView(bytes.buffer, bytes.byteOffset).getUint32(0x24, true);

    // EF_ARM_EABI_VER5 = 0x05000000
    // Check for Thumb entry point bit (bit 0 of entry point address at offset 0x18)
    const entryPoint = new DataView(bytes.buffer, bytes.byteOffset).getUint32(0x18, true);
  ...
  const handleDisassembleELF = async () => {
    ...
    try {
      ...
      const codeBytes = bytes.slice(codeStart, Math.min(codeStart + 512, bytes.length));
      addOutput(`Disassembling ${codeBytes.length} bytes from offset 0x${codeStart.toString(16)} in ${useMode.toUpperCase()} mode...`);

      const instructions = useMode === 'thumb'
      ...
      instructions.forEach((instr: WasmInstruction, idx: number) => {
        try {
          ...
          addOutput(`  0x${addr}:  ${bytes_hex.padEnd(12)}  ${mnemonic} ${operands}`);
      ...
      <div className="min-h-screen bg-gray-950 text-gray-100 py-16 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-6 mb-6">
            ...
            <div className="mb-4">
              <label className="text-sm text-gray-400 mb-2 block">Instruction Set Mode</label>
            ...
            <div className="flex flex-wrap gap-3 items-center">
              ...
              <button
                ...
                title={!wasm.isInitialized ? "WASM still loading..." : !elfFile ? "Choose a file first" : "Click to disassemble"}
              >
                Disassemble ELF
            ...
            <p className="text-sm text-gray-500 mt-2">
              Upload an ARM ELF binary to disassemble. Select ARM mode for 32-bit instructions or Thumb mode for 16/32-bit mixed instructions.
            </p>
          </div>
