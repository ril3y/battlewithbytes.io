...
interface SessionNotes {
  ...
  entries?: Array<{          // Optional, timestamped entries
...
export function isValidProject(data: unknown): data is BattleMagicProject {
  ...
  if (!project.metadata || !project.gdbSettings ||
      !project.memoryMap || !project.breakpoints) {
    return false;
  }

  // Check metadata
  if (!project.metadata.name || !project.metadata.createdAt ||
