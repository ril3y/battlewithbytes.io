...
import type { AnalysisResults, XrefResult, Loop, VectorTableEntry } from '../wasmAnalyzer';
import { XrefType } from '../wasmAnalyzer';
import { getAnalysisDatabase, type DbFunction, type DbComment, type DbXref, type CommentType } from '../db/AnalysisDatabase';
...
interface AnalysisContextState {
  ...
  // Methods
  setAnalysisResults: (results: AnalysisResults, baseAddr: number, size: number) => Promise<void>;
  clearAnalysis: () => void;
...
export function AnalysisProvider({ children }: { children: React.ReactNode }) {
  ...
  const [comments, setComments] = useState<Map<number, Map<CommentType, Comment>>>(new Map());
  ...
  const setAnalysisResults = useCallback((
    ...
    return Promise.all([
      ...
      if (newResults.functions && userRenamedFunctions.size > 0) {
        ...
        setResults({ ...newResults });
        console.log(`[AnalysisContext] Restored ${userRenamedFunctions.size} function renames`);
      }
      ...
      if (dbComments.length > 0) {
        console.log(`[AnalysisContext] Restored ${dbComments.length} comments`);
      }
    }).catch(error => {
  ...
  const renameFunction = useCallback((address: number, newName: string) => {
    ...
    if (results?.functions) {
      const resultFunc = results.functions.find(f => f.start_address === address);
      if (resultFunc) {
    ...
    db.saveFunction({
      ...
      console.error('[AnalysisContext] Failed to save renamed function:', error);
    });
  }, [functions, results]);

  const getComment = useCallback((address: number, type: CommentType = 'standard'): Comment | null => {
    const commentMap = comments.get(address);
    return commentMap?.get(type) || null;
  }, [comments]);

  const getCommentsAt = useCallback((address: number): Map<CommentType, Comment> => {
    return comments.get(address) || new Map();
  }, [comments]);

  const setComment = useCallback((address: number, text: string, type: CommentType) => {
    setComments(prev => {
      ...
      const commentMap = oldCommentMap ? new Map(oldCommentMap) : new Map<CommentType, Comment>();
  ...
  const getArgAnnotation = useCallback((callAddress: number): ArgAnnotation | null => {
    ...
    for (const func of functions.values()) {
      if (func.arg_annotations && func.arg_annotations.length > 0) {
        const annotation = func.arg_annotations.find((ann: ArgAnnotation) => ann.call_address === callAddress);
        if (annotation) {
          return annotation;
        }
      }
  ...
  const saveToDatabase = useCallback(async () => {
    ...
    try {
      ...
      const dbVectorTable = results.vector_table?.map(entry => ({
      ...
      if (projectContext) {
        ...
        console.log('[AnalysisContext] Updated and saved project with analysis metadata');
      }
    } catch (error) {
  ...
  const loadFromDatabase = useCallback(async (): Promise<boolean> => {
    try {
      ...
      const [dbFunctions, dbComments, dbXrefs, dbVectorTable, dbBaseAddress, dbFirmwareSize, dbTotalInstructions, dbAnalysisTime] = await Promise.all([
        db.getAllFunctions(),
        db.getAllComments(),
        db.getAllXrefs(),
  ...
  const clearAutoGeneratedData = useCallback(async () => {
    try {
      ...
      console.log('[AnalysisContext] Cleared auto-generated data, preserved user data');
    } catch (error) {
      console.error('[AnalysisContext] Failed to clear auto-generated data:', error);
      throw error;
    }
  }, []);
  ...
  // CRITICAL FIX: Memoize context value to prevent excessive re-renders
  // Without this, the value object is recreated on every render, causing cascading re-renders
  // This was causing FirmwareDumpWorkflow to re-render 8x consecutively
  const value = useMemo<AnalysisContextState>(() => ({
    results,
    baseAddress,
    firmwareSize,
    xrefsTo,
