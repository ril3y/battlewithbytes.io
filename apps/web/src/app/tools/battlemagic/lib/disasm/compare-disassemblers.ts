...
const TEST_SAMPLES = {
  ...
  simpleLoop: new Uint8Array([
    ...
    0x01, 0x30,             // ADDS r0, #1
    0x0a, 0x28,             // CMP r0, #10
    0xfc, 0xd1,             // BNE (back to ADDS)
    0x70, 0x47,             // BX lr
  ]),

  // Function call sequence
  functionCall: new Uint8Array([
    0x00, 0xf0, 0x04, 0xf8, // BL #8 (call function)
    0x00, 0x20,             // MOVS r0, #0
    0x70, 0x47,             // BX lr
  ...
  wideInstructions: new Uint8Array([
    ...
    0x4f, 0xf4, 0x80, 0x70, // MOV.W r0, #256
    0x00, 0xf0, 0x00, 0xb8, // B.W #0
  ]),

  // Mixed 16-bit and 32-bit
  mixedInstructions: new Uint8Array([
    0x00, 0x20,             // MOVS r0, #0 (16-bit)
    0x4f, 0xf0, 0xff, 0x01, // MOV.W r1, #255 (32-bit)
    0x08, 0x44,             // ADD r0, r1 (16-bit)
    0x70, 0x47,             // BX lr (16-bit)
  ]),
};
...
function formatInstruction(inst: DisassembledInstruction): string {
  ...
  const target = inst.branchTarget ? `-> 0x${inst.branchTarget.toString(16).toUpperCase()}` : '';
...
async function compareSample(
  ...
  const capstoneInstructions = await capstoneDisasm.disassemble(code, baseAddress, true);
  ...
  console.log(`CapstoneDisassembler: ${capstoneInstructions.length} instructions`);

  const armBranches = armInstructions.filter(i => i.isBranch).length;
  const capstoneBranches = capstoneInstructions.filter(i => i.isBranch).length;
  console.log(`Branches detected: ARM=${armBranches}, Capstone=${capstoneBranches}`);

  const armTargets = armInstructions.filter(i => i.branchTarget !== undefined).length;
  const capstoneTargets = capstoneInstructions.filter(i => i.branchTarget !== undefined).length;
...
async function performanceTest(iterations: number = 1000): Promise<void> {
  ...
  console.log(`ArmDisassembler:      ${armTime.toFixed(2)}ms (${(armTime / iterations).toFixed(3)}ms per call)`);
  console.log(`CapstoneDisassembler: ${capstoneTime.toFixed(2)}ms (${(capstoneTime / iterations).toFixed(3)}ms per call)`);
...
export async function runComparison(): Promise<void> {
  ...
  console.log('='.repeat(80));
  console.log('\nKey Differences:');
  console.log('1. CapstoneDisassembler has complete ARM Thumb/Thumb-2 coverage');
  console.log('2. CapstoneDisassembler detects more branch types (CBZ, CBNZ, etc.)');
  console.log('3. CapstoneDisassembler provides more accurate mnemonics');
  console.log('4. ArmDisassembler is slightly faster (pure JavaScript vs WebAssembly)');
  console.log('5. CapstoneDisassembler requires async initialization');
  console.log('\nRecommendation:');
  console.log('- Use CapstoneDisassembler for production/analysis (better accuracy)');
 ...
 */
export async function quickCompare(hexString: string, address: number = 0x08000000): Promise<void> {
