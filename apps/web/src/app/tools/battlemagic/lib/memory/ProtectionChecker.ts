...
export class ProtectionChecker {
  ...
  private async checkStm32Protection(): Promise<ProtectionInfo> {
    try {
      ...
      } else if (optionOutput.toLowerCase().includes('rdp level 2') ||
      ...
      return {
        status: canRead ? ('None' as ProtectionStatus) : ('Unknown' as ProtectionStatus),
        details: canRead
  ...
  private async checkNrfProtection(flashBase: number): Promise<ProtectionInfo> {
    try {
      ...
      const status: ProtectionStatus = (!canRead || approtectEnabled)
   ...
   */
  private async checkEsp32Protection(flashBase: number): Promise<ProtectionInfo> {
    const canRead = await this.testRead(flashBase);

    return {
      status: canRead ? ('None' as ProtectionStatus) : ('Read Protected' as ProtectionStatus),
      details: canRead
   ...
   */
  private async checkGenericProtection(flashBase: number): Promise<ProtectionInfo> {
    const canRead = await this.testRead(flashBase);
   ...
   */
  private getStm32ProtectionDetails(rdpLevel: RdpLevel, canRead: boolean): string {
   ...
   */
  private getNrfProtectionDetails(approtectEnabled: boolean, canRead: boolean): string {
