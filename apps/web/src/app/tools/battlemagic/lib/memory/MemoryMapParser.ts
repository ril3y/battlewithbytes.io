...
const ARM_MEMORY_MAP = {
  CODE: { start: 0x00000000, end: 0x1FFFFFFF, name: 'Code', type: MemoryType.FLASH },
  SRAM: { start: 0x20000000, end: 0x3FFFFFFF, name: 'SRAM', type: MemoryType.SRAM },
  PERIPHERAL: { start: 0x40000000, end: 0x5FFFFFFF, name: 'Peripheral', type: MemoryType.PERIPHERAL },
  EXTERNAL_RAM: { start: 0x60000000, end: 0x9FFFFFFF, name: 'External RAM', type: MemoryType.EXTERNAL_RAM },
  EXTERNAL_DEVICE: { start: 0xA0000000, end: 0xDFFFFFFF, name: 'External Device', type: MemoryType.EXTERNAL_DEVICE },
  SYSTEM: { start: 0xE0000000, end: 0xFFFFFFFF, name: 'System', type: MemoryType.SYSTEM }
};
...
export class MemoryMapParser {
  ...
  parseGdbMemoryInfo(output: string): MemoryRegion[] {
    ...
    for (const line of lines) {
      const match = line.match(/Memory region\s+(0x[0-9a-fA-F]+)-(0x[0-9a-fA-F]+),?\s*(\w+)?,?\s*([rwx-]+)?/);
      if (match) {
  ...
  calculateStatistics(regions?: MemoryRegion[]): MemoryStatistics {
    ...
    for (const region of regs) {
      ...
      } else if (region.type === MemoryType.RAM || region.type === MemoryType.SRAM) {
   ...
   */
  updateMemoryUsage(symbolData: Map<string, number>, stackPointer?: number, heapInfo?: { start: number; end: number }) {
    ...
    // Find vector table
    const vectorTable = symbolData.get('g_pfnVectors') || symbolData.get('__isr_vector');
    if (vectorTable) {
    ...
    for (const region of this.regions) {
      if (region.type === MemoryType.FLASH) {
        ...
        if (stackPointer && stackPointer >= region.start && stackPointer <= region.end) {
          region.used = region.end - stackPointer;
        }
        if (heapInfo && heapInfo.start >= region.start && heapInfo.end <= region.end) {
