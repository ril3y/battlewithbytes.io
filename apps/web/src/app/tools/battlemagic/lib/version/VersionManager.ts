...
export class VersionManager {
  ...
  private parsedVersion: { major: number; minor: number; patch: number } | null = null;
   ...
   */
  private parseVersionString(versionStr: string): { major: number; minor: number; patch: number } | null {
  ...
  isVersionSupported(): boolean {
    ...
    return this.compareVersions(this.version.firmware, VersionManager.MIN_REQUIRED_VERSION) >= 0;
  }
  ...
  isFeatureAvailable(featureId: string): FeatureStatus {
    ...
    if (this.version.firmware.includes('-') || this.version.firmware.includes('git')) {
  ...
  getAvailableFeatures(): BmpFeature[] {
    ...
    return BMP_FEATURES.filter(feature => {
      ...
      return status === FeatureStatus.SUPPORTED || status === FeatureStatus.EXPERIMENTAL;
    });
  }
  ...
  getCompatibilityReport(): {
    ...
    const isLatest = this.compareVersions(this.version.firmware, VersionManager.LATEST_STABLE_VERSION) >= 0;
