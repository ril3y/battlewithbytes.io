...
export class MockCapstone {
  ...
  private decodeThumb32(
    ...
    if ((halfword1 & 0xf800) === 0xf000 && (halfword2 & 0xd000) === 0xd000) {
      ...
      let offset = (s ? -1 << 24 : 0) | (i1 << 23) | (i2 << 22) | (imm10 << 12) | (imm11 << 1);
    ...
    // B.W instruction
    else if ((halfword1 & 0xf800) === 0xf000 && (halfword2 & 0xd000) === 0x9000) {
      const s = (halfword1 >> 10) & 1;
      const j1 = (halfword2 >> 13) & 1;
      const j2 = (halfword2 >> 11) & 1;
      const imm10 = halfword1 & 0x3ff;
      ...
      const offset = (s ? -1 << 20 : 0) | (j2 << 19) | (j1 << 18) | (imm10 << 12) | (imm11 << 1);
      const target = address + 4 + offset;
      mnemonic = 'b.w';
      op_str = `#0x${target.toString(16)}`;
    }
    // PUSH (32-bit form)
    else if ((halfword1 & 0xfff0) === 0xe920 && (halfword2 & 0x8000) === 0x0000) {
      mnemonic = 'push';
    ...
    // POP (32-bit form)
    else if ((halfword1 & 0xfff0) === 0xe8b0 && (halfword2 & 0x0000) === 0x0000) {
      mnemonic = 'pop';
      const reglist = halfword2;
      const regs = this.formatRegList(reglist);
      op_str = `{${regs}}`;
  ...
  private formatRegList(bitmap: number): string {
    ...
    for (let i = 0; i < 16; i++) {
      if (bitmap & (1 << i)) {
        regs.push(i === 13 ? 'sp' : i === 14 ? 'lr' : i === 15 ? 'pc' : `r${i}`);
      }
    }
    return regs.join(', ');
  }
