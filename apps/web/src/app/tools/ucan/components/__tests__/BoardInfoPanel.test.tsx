...
jest.mock("../../utils/deviceStorage", () => ({
  ...
  formatDeviceName: jest.fn(
    (device: { vendorId?: number }) => `Device ${device?.vendorId}`,
  ),
...
describe("BoardInfoPanel", () => {
  ...
  describe("Connection State", () => {
    test('renders "No board connected" when not connected', () => {
      ...
      expect(
        screen.getByText("Connect to a uCAN device to view capabilities"),
      ).toBeInTheDocument();
    });
    ...
    test('renders "Loading capabilities" when connected but no capabilities', () => {
      ...
      expect(
        screen.getByText("Loading board capabilities..."),
      ).toBeInTheDocument();
    });
  ...
  describe("max_rules Display - Regression Tests", () => {
    ...
    test('displays "Loading..." when max_rules is undefined', () => {
      ...
      render(
        <BoardInfoPanel
          {...defaultProps}
          capabilities={capsWithoutMaxRules as BoardCapabilities}
        />,
      );
  ...
  describe("Device Name Editing", () => {
    test("allows editing device name", async () => {
      ...
      render(
        <BoardInfoPanel {...defaultProps} onSendCommand={onSendCommand} />,
      );
  ...
  describe("Rule Management", () => {
    ...
    test("calls onClearAllRules with confirmation", async () => {
      ...
      render(
        <BoardInfoPanel {...defaultProps} onClearAllRules={onClearAllRules} />,
      );
  ...
  describe("CAN Configuration", () => {
    test("allows changing CAN bitrate", async () => {
      ...
      // Find bitrate select
      const bitrateSelect = screen
        .getByText("Change bitrate...")
        .closest("select");
    ...
    test("allows setting hardware filter", async () => {
      ...
      // Find filter inputs
      const filterIdInput = screen.getByPlaceholderText(
        "Filter ID (e.g., 0x500)",
      );
  ...
  describe("Accessibility", () => {
    ...
    test("tooltips provide additional context", () => {
      ...
      const canTooltip = tooltips.find((t) =>
        t.getAttribute("title")?.includes("CAN Controller Info"),
      );
