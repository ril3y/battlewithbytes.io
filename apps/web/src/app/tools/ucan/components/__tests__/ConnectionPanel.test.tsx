...
describe('ConnectionPanel', () => {
  ...
  describe('Browser Compatibility', () => {
    test('shows warning when Web Serial API is not supported', () => {
      ...
      expect(screen.getByText(/Please use Chrome, Edge, or Opera browser/)).toBeInTheDocument();
    });
    ...
    test('renders connection UI when Web Serial API is supported', () => {
      ...
      expect(screen.queryByText('Web Serial API Not Supported')).not.toBeInTheDocument();
  ...
  describe('Connection State - Disconnected', () => {
    test('shows connect button when disconnected', () => {
      ...
      const connectButton = screen.getByRole('button', { name: /Connect to New Device/i });
    ...
    test('calls onConnect when connect button clicked', async () => {
      ...
      await waitFor(() => {
        expect(onConnect).toHaveBeenCalledWith(undefined, DEFAULT_SERIAL_CONFIG);
      });
    });
  ...
  describe('Quick Reconnect - Last Device', () => {
    ...
    test('shows warning when last device not currently available', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Last Device Not Available/i)).toBeInTheDocument();
        expect(screen.getByText(/My uCAN Board is not currently connected/i)).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Other Authorized Ports', () => {
    test('displays other previously connected devices', async () => {
      ...
      mockSerialBridge.getAuthorizedPorts.mockResolvedValue([mockPort1, mockPort2]);

      render(<ConnectionPanel {...defaultProps} />);

      await waitFor(() => {
        expect(screen.getByText(/Previously Connected:/i)).toBeInTheDocument();
        expect(screen.getByText(/VID: 0x2E8A, PID: 0x000A/i)).toBeInTheDocument();
        expect(screen.getByText(/VID: 0x1234, PID: 0x5678/i)).toBeInTheDocument();
      });
    });
  ...
  describe('Serial Configuration', () => {
    ...
    test('allows changing baud rate', async () => {
      ...
      render(<ConnectionPanel {...defaultProps} onConfigChange={onConfigChange} />);
