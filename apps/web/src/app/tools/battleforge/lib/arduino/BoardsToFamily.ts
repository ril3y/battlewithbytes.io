...
function extractFamilyId(
  ...
  // nRF families
  if (archLower.includes("nrf52") || mcuLower.includes("nrf52"))
    return "nrf52";
  if (archLower.includes("nrf53") || mcuLower.includes("nrf53"))
    return "nrf53";
...
function transformBoard(
  ...
  const linkerScript =
    board.build?.ldscript || `${board.id}.ld`;
...
export class BoardsToFamilyTransformer {
  ...
  static transform(platform: ParsedArduinoPlatform): PlatformFamily | null {
    ...
    if (devices.length === 0) {
      console.warn(
        `No valid devices found for ${platform.platformName}`
      );
      return null;
    }
