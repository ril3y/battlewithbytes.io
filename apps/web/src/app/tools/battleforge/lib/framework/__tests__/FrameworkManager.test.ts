...
describe('FrameworkManager', () => {
  ...
  describe('loadFramework', () => {
    it('should load framework definition from server', async () => {
      ...
      const framework = await frameworkManager.loadFramework('arduino', 'stm32', 'f1');
      ...
      expect(fetchMock).toHaveBeenCalledWith('/platforms/stm32/f1/frameworks/arduino.json');
    });
    ...
    it('should cache framework definition in memory', async () => {
      ...
      // First load
      const framework1 = await frameworkManager.loadFramework('arduino', 'stm32', 'f1');
      expect(fetchMock).toHaveBeenCalledTimes(1);

      // Second load - should use cache
      const framework2 = await frameworkManager.loadFramework('arduino', 'stm32', 'f1');
    ...
    it('should return null if framework not found', async () => {
      ...
      const framework = await frameworkManager.loadFramework('mbed', 'stm32', 'f1');
    ...
    it('should load different frameworks for different platforms', async () => {
      const arduinoFramework = { ...mockFramework, id: 'arduino' as FrameworkId };
      const mbedFramework = { ...mockFramework, id: 'mbed' as FrameworkId, name: 'Mbed OS' };
      ...
      const framework1 = await frameworkManager.loadFramework('arduino', 'stm32', 'f1');
      const framework2 = await frameworkManager.loadFramework('mbed', 'stm32', 'f1');
  ...
  describe('loadCoreFiles', () => {
    it('should throw error if framework has no core files', async () => {
      const frameworkWithoutCore = { ...mockFramework, coreUrl: undefined, coreChecksum: undefined };
    ...
    it('should download and extract core files', async () => {
      ...
      const mockDecompressedStream = {
        getReader: () => ({
          read: vi.fn()
            .mockResolvedValueOnce({ done: false, value: new Uint8Array(512).fill(0) })
            .mockResolvedValueOnce({ done: true, value: undefined })
        })
      };
      ...
      try {
        ...
        const onProgress = vi.fn((progress) => progressCallbacks.push(progress));
        ...
        expect(fetchMock).toHaveBeenCalledWith('/platforms/stm32/f1/frameworks/arduino-core.tar.gz');
    ...
    it('should report download progress', async () => {
      ...
      const downloadProgress = progressUpdates.find(p => p.stage === 'downloading' && p.current);
  ...
  describe('getCacheSize', () => {
    it('should return cache size for framework', async () => {
      const size = await frameworkManager.getCacheSize('arduino', 'stm32', 'f1');
  ...
  describe('Memory Management', () => {
    it('should handle multiple framework loads efficiently', async () => {
      ...
      const results = await Promise.all(
        frameworks.map(fw => frameworkManager.loadFramework(fw, 'stm32', 'f1'))
      );
