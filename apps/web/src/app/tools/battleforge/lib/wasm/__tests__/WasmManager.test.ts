...
describe("WASM Manifest", () => {
  ...
  describe("Compiler Definitions", () => {
    ...
    it("should have valid Xtensa compiler definition", () => {
      const xtensa = mockManifest.compilers.find((c) => c.id === "clang-xtensa");
...
describe("Compiler Platform Mapping", () => {
  ...
  const platformConfigs: PlatformConfig[] = [
    // STM32 platforms
    { platform: "stm32", family: "f1", architecture: "cortex-m3", expectedCompiler: "clang-arm" },
    { platform: "stm32", family: "f4", architecture: "cortex-m4f", expectedCompiler: "clang-arm" },
    { platform: "stm32", family: "l4", architecture: "cortex-m4f", expectedCompiler: "clang-arm" },
    { platform: "stm32", family: "h7", architecture: "cortex-m7f", expectedCompiler: "clang-arm" },

    // Nordic platforms
    { platform: "nrf", family: "nrf52", architecture: "cortex-m4f", expectedCompiler: "clang-arm" },
    { platform: "nrf", family: "nrf53", architecture: "cortex-m33", expectedCompiler: "clang-arm" },

    // RP2040
    { platform: "rp2040", family: "rp2", architecture: "cortex-m0+", expectedCompiler: "clang-arm" },

    // ESP32 Xtensa
    { platform: "esp32", family: "esp32", architecture: "xtensa-lx6", expectedCompiler: "clang-xtensa" },
    { platform: "esp32", family: "esp32s2", architecture: "xtensa-lx7", expectedCompiler: "clang-xtensa" },
    { platform: "esp32", family: "esp32s3", architecture: "xtensa-lx7", expectedCompiler: "clang-xtensa" },

    // ESP32 RISC-V
    { platform: "esp32", family: "esp32c3", architecture: "riscv32", expectedCompiler: "clang-riscv" },
    { platform: "esp32", family: "esp32c6", architecture: "riscv32", expectedCompiler: "clang-riscv" },
    { platform: "esp32", family: "esp32h2", architecture: "riscv32", expectedCompiler: "clang-riscv" },
  ];

  for (const config of platformConfigs) {
    it(`should use ${config.expectedCompiler} for ${config.platform}/${config.family} (${config.architecture})`, () => {
      const compilerId = getCompilerForArchitecture(config.architecture as import("../../platform/types").Architecture);
  ...
  it("should track storage usage", () => {
    const stats = {
      ...
      compilers: [
        ...
        { id: "clang-xtensa" as CompilerId, size: 35000000, version: "18.1.0-esp" },
      ],
    };
    ...
    const totalFromCompilers = stats.compilers.reduce((sum, c) => sum + c.size, 0);
