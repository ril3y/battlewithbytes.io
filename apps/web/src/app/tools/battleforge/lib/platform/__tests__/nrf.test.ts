...
const mockNrf52Family: PlatformFamily = {
  ...
  description: 'Ultra low power wireless SoC with BLE 5.0 support. Popular for IoT and wearable applications.',
...
describe('nRF52 Family Schema', () => {
  ...
  it('should have correct compiler flags for Cortex-M4F', () => {
    expect(mockNrf52Family.compilerFlags).toContain('--target=thumbv7em-none-eabihf');
...
describe('nRF53 Family Schema', () => {
  ...
  it('should have correct compiler flags for Cortex-M33', () => {
    expect(mockNrf53Family.compilerFlags).toContain('--target=thumbv8m.main-none-eabihf');
...
describe('Zephyr Framework Definition', () => {
  const zephyrFramework: Framework = {
    ...
    version: '3.5.0',
    compilerFlags: ['-std=c11', '-fno-common', '-ffunction-sections', '-fdata-sections'],
    linkerFlags: ['--gc-sections'],
    defines: ['__ZEPHYR__', 'CONFIG_NRF52', 'CONFIG_BT=1'],
    includePaths: ['/zephyr/include', '/zephyr/arch/arm', '/zephyr/subsys/bluetooth'],
