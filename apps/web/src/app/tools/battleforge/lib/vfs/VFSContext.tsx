...
interface VFSContextType {
  state: VFSState;
  // File operations
  addFile: (path: string, content: string | Uint8Array, editable?: boolean) => void;
  ...
  // Bulk operations
  addFilesFromMap: (files: Map<string, string | Uint8Array>, editable?: boolean) => void;
  clearProject: () => void;
...
export function VFSProvider({ children }: { children: ReactNode }) {
  ...
  const findNode = useCallback((root: VFSDirectory, path: string): VFSNode | null => {
    ...
    for (const part of parts) {
      ...
      const child: VFSNode | undefined = current.children.find(c => c.name === part);
  ...
  const findParentDirectory = useCallback((root: VFSDirectory, path: string): VFSDirectory | null => {
  ...
  const updateNodeInTree = useCallback((
    ...
    const updateChildren = (dir: VFSDirectory, remainingParts: string[]): VFSDirectory => {
  ...
  const addFile = useCallback((path: string, content: string | Uint8Array, editable = true) => {
    setState(prev => {
      ...
      const addFileRecursive = (
        ...
        const existingDir = dir.children.find(c => c.name === part && isDirectory(c));
          ...
          return {
            ...
            children: dir.children.map(child => {
              if (child === existingDir) {
                return addFileRecursive(existingDir, pathParts.slice(1), fullPath);
              }
              return child;
            }),
          };
        ...
        const populatedDir = addFileRecursive(newDir, pathParts.slice(1), fullPath);
  ...
  const updateFile = useCallback((path: string, content: string | Uint8Array) => {
    setState(prev => {
  ...
  const markFileSaved = useCallback((path: string) => {
    setState(prev => {
      const newRoot = updateNodeInTree(prev.root, path, node => {
        if (isFile(node)) {
  ...
  const deleteFile = useCallback((path: string) => {
    setState(prev => {
      ...
      const newOpenFiles = prev.openFiles.filter(f => f !== path && !f.startsWith(path + '/'));
      const newActiveFile = prev.activeFile === path || prev.activeFile?.startsWith(path + '/')
        ? (newOpenFiles[0] || null)
        : prev.activeFile;
      const newSelectedPath = prev.selectedPath === path || prev.selectedPath?.startsWith(path + '/')
        ? null
        : prev.selectedPath;
  ...
  const getFile = useCallback((path: string): VFSFile | null => {
  ...
  const renameFile = useCallback((oldPath: string, newName: string) => {
    setState(prev => {
      ...
      const newOpenFiles = prev.openFiles.map(f => f === oldPath ? newPath : f);
      const newActiveFile = prev.activeFile === oldPath ? newPath : prev.activeFile;
      const newSelectedPath = prev.selectedPath === oldPath ? newPath : prev.selectedPath;
  ...
  const moveFile = useCallback((sourcePath: string, targetDirPath: string) => {
    setState(prev => {
      ...
      if (isDirectory(sourceNode) && targetDirPath.startsWith(sourcePath + '/')) return prev;
      ...
      const updatePaths = (node: VFSNode, oldBasePath: string, newBasePath: string): VFSNode => {
      ...
      const rootAfterRemove = updateNodeInTree(prev.root, sourcePath, () => null);

      // Add to new location
      const addToTarget = (dir: VFSDirectory, targetPath: string): VFSDirectory => {
      ...
      const newActiveFile = prev.activeFile ? updatePathRef(prev.activeFile) : null;
      const newSelectedPath = prev.selectedPath ? updatePathRef(prev.selectedPath) : null;
  ...
  const addDirectory = useCallback((path: string) => {
    setState(prev => {
      ...
      const addDirToParent = (dir: VFSDirectory, pathParts: string[], currentPath: string): VFSDirectory => {
        if (pathParts.length === 0) return dir;
  ...
  const deleteDirectory = useCallback((path: string) => {
    setState(prev => {
      ...
      const newOpenFiles = prev.openFiles.filter(f => !f.startsWith(path + '/') && f !== path);
      const newActiveFile = prev.activeFile && (prev.activeFile.startsWith(path + '/') || prev.activeFile === path)
        ? (newOpenFiles[0] || null)
        : prev.activeFile;
      const newSelectedPath = prev.selectedPath && (prev.selectedPath.startsWith(path + '/') || prev.selectedPath === path)
        ? null
        : prev.selectedPath;
  ...
  const hasDirectory = useCallback((path: string): boolean => {
    const node = findNode(state.root, path);
  ...
  const closeFile = useCallback((path: string) => {
    setState(prev => {
      ...
      const newActiveFile = prev.activeFile === path
  ...
  const getFilesForCompiler = useCallback((): Record<string, string | Uint8Array> => {
