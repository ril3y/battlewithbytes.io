...
import type { LibraryRegistryEntry, PlatformId, Architecture } from '../lib/library';
...
export function LibraryPanel({ platformId, architecture, onLog, onLibraryFilesChanged, onLibraryUninstalled }: LibraryPanelProps) {
  const {
    registryLibraries,
    getAllLibraryFiles,
    installedLibraries,
  ...
  const handleInstall = useCallback(async (lib: LibraryRegistryEntry) => {
    if (!architecture) {
  ...
  }, [installFromRegistry, architecture, onLog, onLibraryFilesChanged, getAllLibraryFiles]);
  ...
  const isLibraryInstalled = useCallback((libId: string) => {
    return installedLibraries.some(i =>
  ...
  return (
    <div className="library-panel" style={{
      display: 'flex',
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            background: 'none',
            border: 'none',
            color: activeTab === 'browse' ? 'var(--accent-color, #0ea5e9)' : 'inherit',
            borderBottom: activeTab === 'browse' ? '2px solid var(--accent-color, #0ea5e9)' : '2px solid transparent',
            cursor: 'pointer'
          }}
        >
          Browse Libraries
        ...
        <button
          ...
          style={{
            ...
            background: 'none',
            border: 'none',
            color: activeTab === 'installed' ? 'var(--accent-color, #0ea5e9)' : 'inherit',
            borderBottom: activeTab === 'installed' ? '2px solid var(--accent-color, #0ea5e9)' : '2px solid transparent',
            cursor: 'pointer'
          }}
        >
      ...
      {activeTab === 'browse' && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {/* Progress / Error */}
          {progress && (
            <div style={{ padding: '4px 8px', color: 'var(--text-secondary, #888)', fontSize: '12px' }}>
              {progress}
            </div>
          )}
          {error && (
            <div style={{ padding: '4px 8px', color: 'var(--error-color, #f87171)', fontSize: '12px' }}>
              {error}
            </div>
          )}
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: '12px', color: 'var(--text-secondary, #888)', margin: '4px 0' }}>
            v{library.version}
          </div>
        ...
        <button
          ...
          style={{
            ...
            color: 'white',
            fontSize: '12px',
            cursor: isInstalled || !hasArchitecture ? 'default' : isInstalling ? 'wait' : 'pointer',
