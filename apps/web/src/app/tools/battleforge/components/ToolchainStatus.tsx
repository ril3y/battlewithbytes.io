    ...
    >
      <style jsx>{`
        .status-indicator {
          ...
          height: 8px;
          border-radius: 50%;
          animation: ${stage === 'downloading' || stage === 'extracting' ? 'pulse 1.5s infinite' : 'none'};
        }
        @keyframes pulse {
          0%, 100% { opacity: 1; }
          50% { opacity: 0.4; }
...
export function ToolchainStatus({ state, compact = false }: ToolchainStatusProps) {
  const items = [
  ...
  const allReady = items.every(item => item.state.stage === 'ready');
    ...
    return (
      <div className="toolchain-status-compact">
        <div className="status-summary">
          <StatusIndicator stage={hasError ? 'error' : allReady ? 'ready' : isLoading ? 'downloading' : 'idle'} />
          <span className="status-text">
            {hasError ? 'Error' : allReady ? 'Ready' : isLoading ? 'Loading...' : 'Not loaded'}
          </span>
        </div>
        <style jsx>{`
  ...
  return (
    <div className="toolchain-status">
      <div className="status-header">
        ...
        <span className={`status-badge ${hasError ? 'error' : allReady ? 'ready' : 'loading'}`}>
          {hasError ? 'Error' : allReady ? 'Ready' : 'Loading'}
        </span>
      </div>
      ...
      <div className="status-items">
        {items.map(item => (
          <div key={item.name} className="status-item">
            ...
            {(item.state.stage === 'downloading' || item.state.stage === 'extracting') && item.state.total > 0 && (
              <div className="item-progress">
                <div
                  className="item-progress-bar"
                  style={{ width: `${(item.state.current / item.state.total) * 100}%` }}
                />
              </div>
            )}
            <div className="item-message">
              {item.state.message || (item.state.stage === 'ready' ? 'Loaded' : 'Waiting...')}
            </div>
          </div>
        ))}
      </div>
