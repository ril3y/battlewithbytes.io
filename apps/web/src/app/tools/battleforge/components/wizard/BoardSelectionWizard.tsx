...
export function BoardSelectionWizard({
  ...
  // Platform display metadata (icons, colors, descriptions)
  const PLATFORM_METADATA: Record<string, { icon: string; color: string; description: string }> = {
    stm32: {
      icon: "/tools/battleforge/platforms/icons/stm32.svg",
      color: "#03234B",
      description: "Industry-leading ARM Cortex-M microcontrollers",
    },
    esp32: {
      icon: "/tools/battleforge/platforms/icons/esp32.svg",
      color: "#E7352C",
      description: "WiFi and Bluetooth SoCs for IoT applications",
    },
    nrf: {
      icon: "/tools/battleforge/platforms/icons/nrf.svg",
      color: "#00A9CE",
      description: "Ultra low power wireless SoCs for BLE",
    },
    rp2040: {
      icon: "/tools/battleforge/platforms/icons/rp2040.svg",
      color: "#C51A4A",
      description: "Dual-core ARM Cortex-M0+ with PIO",
    },
  };
  ...
  const loadRegistry = async () => {
    try {
      setLoading(true);
      // Load from battleforge boards API - contains platforms only
      const response = await fetch("/tools/battleforge/boards/registry.json");
      if (!response.ok) throw new Error("Failed to load registry");
      const data = await response.json();

      // Map platforms with display metadata
      const platformList: PlatformOption[] = (data.platforms || []).map((p: {
        id: string;
        platform?: string;
        name: string;
        indexPath: string;
        ...
        // Use the base platform id (e.g., "esp32" from "esp32-esp32") for metadata lookup
        const basePlatformId = p.platform || p.id.split("-")[0];
        const meta = PLATFORM_METADATA[basePlatformId] || {};
        return {
          id: p.id,
          name: p.name,
          description: meta.description || p.name,
          icon: meta.icon || `/tools/battleforge/platforms/icons/${basePlatformId}.svg`,
  ...
  const loadPlatformBoards = async (platformId: string, indexPath?: string) => {
    try {
      ...
      const response = await fetch(`/tools/battleforge/boards/${path}`);
      if (!response.ok) throw new Error(`Failed to load boards for ${platformId}`);
      const data = await response.json();

      // Transform boards from platform index
      const boardList: BoardIndexEntry[] = (data.boards || []).map((b: BoardIndexEntry) => ({
        ...b,
  ...
  const handleBoardSelect = useCallback(async (board: BoardIndexEntry) => {
    try {
      ...
      const response = await fetch(`/tools/battleforge/boards/${board.path}`);
  ...
  const handleBack = useCallback(() => {
    setState((prev) => {
      const stepIndex = WIZARD_STEPS.findIndex((s) => s.key === prev.currentStep);
      if (stepIndex > 0) {
  ...
  const handleCreateProject = useCallback(() => {
    ...
    // Get files from example or create blank
    const files = example
      ? example.files.map((f) => ({
          path: f.path,
          content: f.content,
  ...
  // but keep filter as safeguard
  const filteredBoards = boards.filter((b) => b.platform === state.selectedPlatformId);
        ...
        <div className="wizard-content">
          ...
          {state.currentStep === "details" && state.selectedBoard && (
            <ProjectDetailsForm
              ...
              onNameChange={(name) => setState((prev) => ({ ...prev, projectName: name }))}
