  ...
  const filteredBoards = useMemo(() => {
    return boards.filter((board) => {
      ...
      if (searchQuery) {
        ...
        const matchesTags = board.tags?.some((t) => t.toLowerCase().includes(query));
      ...
      // Feature filter
      if (selectedFeature && !board.features?.includes(selectedFeature)) return false;
                ...
                {board.features && board.features.length > 0 && (
                  <div className="board-features">
                    ...
                    {board.features.length > 3 && (
                      <span className="feature-more">+{board.features.length - 3}</span>
                    )}
                  </div>
                )}
                {board.exampleCount !== undefined && board.exampleCount > 0 && (
                  <div className="board-examples">
                    {board.exampleCount} example{board.exampleCount !== 1 ? "s" : ""}
                  </div>
                )}
              </div>
            </button>
