...
export function WasmManagerPanel({ onLog }: WasmManagerPanelProps) {
  ...
  const loadCompilerInfo = useCallback(async () => {
    try {
      ...
      const message = e instanceof Error ? e.message : "Failed to load compilers";
      setError(message);
      onLog?.(message, "error");
    } finally {
      setIsLoading(false);
...
function CompilerCard({
  ...
  const isComingSoon =
    (compiler.available as Record<string, unknown> | undefined)?.status === "coming_soon";
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: "11px", marginTop: "6px" }}>
            ...
            {!compiler.installed && compiler.available && !isComingSoon && (
              <span style={{ color: "var(--text-secondary, #888)" }}>
                v{compiler.available.version} ({formatBytes(compiler.available.size)})
              </span>
            )}
          </div>
