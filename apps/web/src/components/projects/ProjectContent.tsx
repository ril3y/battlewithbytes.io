...
const components = {
  ...
  code: (props: { children?: React.ReactNode; className?: string }) => {
    ...
    return <code className={`${className} block overflow-x-auto`} {...props} />;
  },
  pre: (props: ComponentProps) => (
    <pre
      className="bg-gray-900 p-4 rounded-md my-6 overflow-x-auto font-mono text-sm"
      {...props}
    />
  ),
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.trim().startsWith("|") && line.trim().endsWith("|")) {
      if (!inTable) {
        inTable = true;
        tableContent =
          '<table className="min-w-full bg-black/30 border border-gray-700 rounded-md">\n<tbody>\n';
      }
  ...
  useEffect(() => {
    const processMdx = async () => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Error processing MDX content";
