...
export default function Description({
  ...
  const generateMathExplanation = () => {
    ...
    return (
      <div className="math-explanation bg-black/50 p-4 rounded-lg border border-gray-800 font-mono text-sm mt-4">
        <h4 className="text-green-400 mb-2 text-lg">
        ...
        {conducting ? (
          <>
            <div className="mb-3">
              ...
              {mosfetType === "n-channel" ? (
                <div className="text-white">
                  Id = Vcc / (Rds_on + {loadResistance?.toFixed(2) ?? "?"}) ={" "}
                  {idNum.toFixed(4)} A
                </div>
              ) : (
                <div className="text-white">
                  Id = Vs / (Rds_on + {loadResistance?.toFixed(2) ?? "?"}) ={" "}
                  {idNum.toFixed(4)} A
                </div>
              )}
            </div>
            ...
            <div className="mb-3">
              ...
              <div className="text-white">
                Vload = Id × Rload ({loadResistance?.toFixed(2) ?? "?"}) ={" "}
                {voltageAcrossLoadNum.toFixed(4)} V
              </div>
            </div>
            ...
            <div className="mb-3">
              ...
              {mosfetType === "n-channel" ? (
                <div className="text-white">
                  Vd (N-Ch) = Vcc - Vload = {vdNum.toFixed(4)} V
                </div>
              ) : (
                <div className="text-white">
                  Vd = Vload = {vdNum.toFixed(4)} V
                </div>
            ...
            <div className="mb-3">
              ...
              <div className="text-white">
                P = Id² × Rds_on (
                {parseFloat(mosfetDetails?.rds_on || "0").toFixed(2)}Ω) = (
                {idNum.toFixed(4)} A)² × Rds_on ={" "}
                {(
                  idNum *
                  idNum *
                  parseFloat(mosfetDetails?.rds_on || "0")
                ).toFixed(6)}{" "}
                W
                {idNum * idNum * parseFloat(mosfetDetails?.rds_on || "0") <
                  0.001 &&
                  ` (${(idNum * idNum * parseFloat(mosfetDetails?.rds_on || "0") * 1000).toFixed(2)} mW)`}
              </div>
            </div>
          </>
        ) : (
          <div className="mb-3">
            <div className="text-gray-400">
              Since Vgs {vgsNum < 0 ? ">" : "<"} Vth, the MOSFET is in cutoff
              mode:
            </div>
  ...
  return (
    <div className="description-container">
      ...
      <div
        className={`mosfet-output mb-4 ${conducting !== null ? (conducting ? "conducting" : "not-conducting") : ""}`}
      >
        {description}
      </div>
          ...
          <div className="results-grid">
            ...
            {powerDissipated && (
              <div className="result-item">
                ...
                <div className="result-value text-green-400">
                  {powerDissipated}
                </div>
              </div>
            )}
