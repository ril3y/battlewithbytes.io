...
const Description: React.FC<DescriptionProps> = ({ results }) => {
    ...
    return (
      <div className="ohms-law-description p-6 bg-black/30 border border-gray-800 rounded-lg">
        ...
        <p className="text-gray-300 font-mono">
          {results.description ||
            "Enter values for two parameters and click 'Calculate' to see results."}
        </p>
      </div>
    );
  }
  ...
  const voltageDisplay = isNaN(voltageNumeric)
    ? "Invalid"
    : formatValueWithSuffix(voltageNumeric, "V");
  const resistanceDisplay = isNaN(resistanceNumeric)
    ? "Invalid"
    : formatValueWithSuffix(resistanceNumeric, "Ω");
  const powerDisplay = isNaN(powerNumeric)
    ? "Invalid"
    : formatValueWithSuffix(powerNumeric, "W");
  ...
  const isCalculated = (property: string) =>
    results.calculatedProperty === property;
  ...
  return (
    <div className="ohms-law-description p-6 bg-black/30 border border-gray-800 rounded-lg">
      ...
      <div className="mt-6">
        <h4 className="text-lg font-bold mb-2 text-[#00ff9d]">
          Calculation Details
        </h4>
        <div className="bg-black/40 border border-gray-800 rounded-md p-4 font-mono text-sm text-gray-300">
          ...
          <div className="mt-4 pt-4 border-t border-gray-800">
            <h5 className="text-sm font-bold mb-2 text-[#00ff9d]">
              Ohm&apos;s Law Formulas Used
            </h5>
            <ul className="list-disc list-inside space-y-1 text-gray-400">
              ...
              >
                Voltage (V) = Current (I) × Resistance (R)  |  V = P / I  |  V =
                √(P × R)
              </li>
              <li
                className={
              ...
              >
                Current (I) = Voltage (V) ÷ Resistance (R)  |  I = P / V  |  I =
                √(P / R)
              </li>
              <li
                className={
              ...
              >
                Resistance (R) = Voltage (V) ÷ Current (I)  |  R = V² / P  |  R
                = P / I²
              </li>
              <li
                className={
                  results.calculatedProperty === "power" ? "text-[#00ff9d]" : ""
                }
              >
                Power (P) = Voltage (V) × Current (I)  |  P = V² / R  |  P = I²
                × R
              </li>
            </ul>
          </div>
