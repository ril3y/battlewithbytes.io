...
const InteractiveCodeBlock: React.FC<InteractiveCodeBlockProps> = ({
  ...
  const [effectiveStartLine, setEffectiveStartLine] = useState<
    number | undefined
  >(startLine);
  const [effectiveEndLine, setEffectiveEndLine] = useState<number | undefined>(
    endLine,
  );
  ...
  const extractLineInfo = (
    ...
    if (match) {
      ...
      const extractedEndLine = match[2]
        ? parseInt(match[2]) - 1
        : extractedStartLine;
  ...
  const getFilteredContent = () => {
    ...
    const end =
      effectiveEndLine !== undefined ? effectiveEndLine : allLines.length - 1;
