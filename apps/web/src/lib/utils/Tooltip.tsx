...
const Tooltip: React.FC<TooltipProps> = ({
  ...
  const checkChildFocus = () => {
    if (childRef.current) {
      const inputs = childRef.current.querySelectorAll(
        "input, textarea, select",
      );
      const hasFocusedInput = Array.from(inputs).some(
    ...
    timeoutRef.current = setTimeout(() => {
      ...
      if (activeTooltipId && activeTooltipId !== tooltipId) {
        ...
        document.dispatchEvent(
          new CustomEvent("closeOtherTooltips", {
  ...
  useEffect(() => {
    ...
    document.addEventListener(
      "closeOtherTooltips",
      handleCloseOtherTooltips as EventListener,
    );
