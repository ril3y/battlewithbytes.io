...
export const formatValueWithSuffix = (
  ...
  // Handle negative numbers - apply same logic to absolute value then add sign
    ...
    if (absFormatted === unit || absFormatted === `0.000${unit}`)
      return `0.000${unit}`;
    return `-${absFormatted}`;
  }
};
 ...
 */
export const createParameterWarningFunction = (
  warningRanges: Record<
    string,
    {
      min?: number;
      max?: number;
  ...
  return (paramName: string, value: string | number): string => {
    ...
    const numValue =
      typeof value === "string" ? parseValueWithSuffix(value) : value;
 ...
 */
export const createParameterTooltipFunction = (
...
export const getParameterWarning = (
  ...
  const numValue =
    typeof value === "string"
      ? paramType === "loadResistance" || paramType === "resistance"
