...
export function FirmwareDumpWorkflow({
  ...
  const [analysisResults, setAnalysisResults] =
    useState<WasmAnalysisResults | null>(null);
  ...
  const [detectedBaseAddress, setDetectedBaseAddress] = useState<number | null>(
    null,
  );
  ...
  const handleUseCachedFirmware = useCallback(async () => {
    ...
    try {
      setProgress({
        stage: "dumping",
        message: "Loading cached firmware...",
        progress: 50,
      });
      ...
      onOutput?.(
        `[Firmware] Loaded cached firmware: ${cachedFirmware.size} bytes`,
      );
      onOutput?.(`[Firmware] Hash: ${cachedFirmware.hash.substring(0, 16)}...`);
      onOutput?.(
        `[Firmware] Cached: ${new Date(cachedFirmware.dumpedAt).toLocaleString()}`,
      );

      // Store firmware in context to prevent unwanted GDB reads
      if (firmwareContext) {
        firmwareContext.setFirmwareData(
          firmwareDump.data,
          firmwareDump.baseAddress,
        );
      ...
      if (autoStart) {
        onOutput?.(
          `[Firmware Analysis] Starting WASM analysis on cached firmware...`,
        );
      ...
      onOutput?.(
        `[Firmware] Error loading cached firmware: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      setProgress({
        stage: "error",
  ...
  const handleDumpFirmware = useCallback(async () => {
    try {
      ...
      if (
        ...
        setProgress({
          stage: "connecting",
          message: "Connecting to Black Magic Probe...",
        });
      ...
      if (currentState !== ConnectionState.ATTACHED) {
        ...
        setProgress({
          stage: "scanning",
          message: "Using already-attached target...",
        });
        // Use detected arch info if available, otherwise use stored target
        if (externalArchInfo) {
          target = {
            id: 1,
            description: externalArchInfo.chip_name,
            type: "unknown",
          };
          ...
          target = {
            id: 1,
            description: lastTarget.description,
            type: "unknown",
          };
          ...
          target = {
            id: 1,
            description: "Unknown (already attached)",
            type: "unknown",
          };
        }
      }
      ...
      if (externalArchInfo) {
        ...
        onOutput?.(
          `[Firmware Dump] Using pre-detected: ${detectedArch.chip_name} (${detectedArch.architecture})`,
        );
      } else if (archInfo) {
        // Reuse previously detected architecture
        detectedArch = archInfo;
        onOutput?.(
          `[Firmware Dump] Using cached: ${detectedArch.chip_name} (${detectedArch.architecture})`,
        );
      } else {
        setProgress({
          stage: "scanning",
          message: "Detecting target architecture...",
        });
        detectedArch = await detectArchitecture(target.description);
        setArchInfo(detectedArch);

        onOutput?.(
          `[Firmware Dump] Detected: ${detectedArch.chip_name} (${detectedArch.architecture})`,
        );
        onOutput?.(
          `[Firmware Dump] Manufacturer: ${detectedArch.manufacturer}`,
        );
        onOutput?.(
          `[Firmware Dump] Analysis Support: ${detectedArch.supported ? "✅ Supported" : "❌ Not Supported"}`,
        );
        onOutput?.(
          `[Firmware Dump] Confidence: ${(detectedArch.confidence * 100).toFixed(1)}%`,
        );
      }
      ...
      if (!detectedArch.supported) {
        console.warn(
          `Architecture ${detectedArch.architecture} not yet supported for analysis`,
        );
        onOutput?.(
          `[Firmware Dump] ⚠️ Architecture ${detectedArch.architecture} not yet supported`,
        );
      }
      ...
      if (currentState !== ConnectionState.ATTACHED) {
        setProgress({
          stage: "attaching",
          message: `Attaching to ${target.description}...`,
        });
      ...
      if (shouldResetBeforeDump) {
        try {
          setProgress({
            stage: "attaching",
            message: "Resetting target to entry point...",
          });
          onOutput?.(
            "[Firmware Dump] Resetting target and halting at entry point...",
          );
          ...
          setProgress({
            stage: "scanning",
            message: "Re-scanning targets after reset...",
          });
          ...
          setProgress({
            stage: "attaching",
            message: "Re-attaching after reset...",
          });
          ...
          setProgress({
            stage: "attaching",
            message: "Reading vector table for base address...",
          });
          onOutput?.(
            "[Firmware Dump] Reading vector table to detect correct base address...",
          );
          ...
          onOutput?.(
            `   Base Address:   0x${vectorInfo.baseAddress.toString(16).toUpperCase().padStart(8, "0")}`,
          );
          onOutput?.(
            `   Initial SP:     0x${vectorInfo.sp.toString(16).toUpperCase().padStart(8, "0")}`,
          );
          onOutput?.(
            `   Reset Handler:  0x${vectorInfo.resetHandler.toString(16).toUpperCase().padStart(8, "0")}`,
          );
          ...
          if (detectedArch) {
            ...
            console.log(
              "[FirmwareDump] Updated flash base to:",
              `0x${vectorInfo.baseAddress.toString(16)}`,
            );
          }
        } catch (error) {
          console.warn(
            "[FirmwareDump] Reset and vector detection failed:",
            error,
          );
          onOutput?.(
            `[Firmware Dump] Warning: Reset failed, using default base address`,
          );
        ...
        try {
          setProgress({
            stage: "attaching",
            message: "Halting target for memory read...",
          });
      ...
      onOutput?.(
        `[Firmware Dump] ✅ Dump complete: ${firmwareDump.size} bytes`,
      );
      if (firmwareDump.vectorTable) {
        onOutput?.(
          `[Firmware Dump] Reset Vector: 0x${firmwareDump.vectorTable.resetAddress.toString(16).toUpperCase()}`,
        );
      }
      ...
      if (firmwareContext) {
        ...
        onOutput?.(
          `[Firmware Dump] Firmware cached in memory for offline analysis`,
        );
      }
      ...
      try {
        ...
        onOutput?.(
          `[Firmware Dump] Firmware cached (hash: ${cached.hash.substring(0, 16)}...)`,
        );
      } catch (error) {
      ...
      onOutput?.(
        `[Firmware Dump] ❌ Error: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      setProgress({
        stage: "error",
        message: `Error: ${error instanceof Error ? error.message : "Unknown error"}`,
  ...
  const handleAnalyzeFirmware = useCallback(async () => {
    ...
    if (archInfo && !archInfo.supported) {
      ...
      console.error(
        `Cannot analyze ${archInfo.chip_name}: ${archInfo.architecture} decoder not implemented`,
      );
      return;
    }
    ...
    try {
      ...
      if (analysisResults) {
        onOutput?.(
          `[Firmware Analysis] Re-analyzing firmware (preserving comments and renames)...`,
        );
      ...
      console.log(
        "[FirmwareDump] Creating analyzer with base address:",
        `0x${dump.baseAddress.toString(16)}`,
      );
      ...
      setProgress({
        stage: "analyzing",
        message: "Analyzing firmware in background...",
      });
      onOutput?.(
        `[Firmware Analysis] Decoding ${dump.data.length} bytes of ${archInfo?.architecture} code...`,
      );
      ...
      const results: any = await new Promise((resolve, reject) => {
        worker.onmessage = (event) => {
          ...
          if (message.type === "progress") {
            ...
            if (
              progressPercent - lastAnalysisUpdate >= 5 ||
              progressPercent === 100 ||
              progressPercent === 0
            ) {
              setProgress({
            ...
            if (
              ...
              message.stage !== "Decoding instructions"
            ) {
              onOutput?.(
                `[Firmware Analysis] ${message.stage}: ${progressPercent}%`,
              );
            }
      ...
      console.log(
        "[FirmwareDump] Vector table detected:",
      ...
      if (vectorTable && vectorTable.length > 0) {
        const validHandlers = vectorTable.filter(
          (e: VectorTableEntry) => e.is_valid && e.vector_number > 0,
        );
        if (validHandlers.length > 0) {
          onOutput?.(
            `   ✓ Detected ${validHandlers.length} vector table handlers`,
          );
          validHandlers.slice(0, 5).forEach((entry: VectorTableEntry) => {
            onOutput?.(
              `     - ${entry.handler_name}: 0x${entry.handler_address.toString(16).toUpperCase()}`,
            );
          });
          if (validHandlers.length > 5) {
            onOutput?.(
              `     ... and ${validHandlers.length - 5} more handlers`,
            );
          }
        }
      }
      ...
      const functionsWithArgs =
        wasmResults.functions?.filter(
          (f: FunctionInfo) =>
            f.arg_annotations && f.arg_annotations.length > 0,
        ).length || 0;
      const totalArgAnnotations =
        wasmResults.functions?.reduce(
          (sum: number, f: FunctionInfo) =>
            sum + (f.arg_annotations?.length || 0),
          0,
        ) || 0;
      ...
      onOutput?.(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
      onOutput?.(
        `   ✓ Decoded ${wasmResults.total_instructions.toLocaleString()} instructions`,
      );
      onOutput?.(
        `   ✓ Found ${wasmResults.xrefs.length.toLocaleString()} cross-references`,
      );
      onOutput?.(
        `   ✓ Detected ${wasmResults.unique_targets.toLocaleString()} unique targets`,
      );
      onOutput?.(
        `   ✓ Detected ${(wasmResults.loops?.length || 0).toLocaleString()} loops`,
      );
      onOutput?.(
        `   ✓ Detected ${(wasmResults.functions?.length || 0).toLocaleString()} functions`,
      );

      // Show vector table stats if available
      if (vectorTable && vectorTable.length > 0) {
        const validCount = vectorTable.filter(
          (e: VectorTableEntry) => e.is_valid && e.vector_number > 0,
        ).length;
      ...
      if (functionsWithArgs > 0) {
        onOutput?.(
          `   ✓ ${functionsWithArgs} function${functionsWithArgs !== 1 ? "s" : ""} with ${totalArgAnnotations} argument annotation${totalArgAnnotations !== 1 ? "s" : ""}`,
        );
      }
      ...
      analysisContext.setAnalysisResults(
        completeResults,
        dump.baseAddress,
        dump.size,
      );
      ...
      try {
        ...
        if (projectContext) {
          ...
          const stateDetails = await stateManager.checkAnalysisState(
            project,
            analysisDb,
          );
          setHasExistingAnalysis(
            stateDetails.state === "current" ||
              stateDetails.state === "outdated",
          );
        ...
        onOutput?.(
          `[Analysis] Warning: Failed to save analysis: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
      ...
      setShowAnalysisModal(false);
      onOutput?.(
        `[Firmware Analysis] ❌ Error: ${error instanceof Error ? error.message : String(error)}`,
      );
  ...
  const handleLoadAnalysis = useCallback(async () => {
    try {
      ...
      setProgress({
        stage: "analyzing",
        message: "Loading analysis from MDB...",
      });
      ...
      onOutput?.(
        `[Analysis] Failed to load: ${error instanceof Error ? error.message : String(error)}`,
      );
      setProgress({
        stage: "error",
  ...
  useEffect(() => {
    const checkAnalysisState = async () => {
      ...
      setHasExistingAnalysis(
        stateDetails.state === "current" || stateDetails.state === "outdated",
      );
      setAnalysisStateMessage(stateDetails.statusMessage);
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-gray-400">
                  Ensures accurate vector table detection and base address
                  identification
                </span>
              </div>
            </label>
              ...
              <div className="flex items-start justify-between">
                <div className="flex flex-col gap-2">
                  ...
                  <div className="text-xs text-gray-400 space-y-1">
                    <div>
                      Size: {cachedFirmware.size.toLocaleString()} bytes (
                      {(cachedFirmware.size / 1024).toFixed(1)} KB)
                    </div>
                    <div>Chip: {cachedFirmware.chipName || "Unknown"}</div>
                    <div>
                      Cached:{" "}
                      {new Date(cachedFirmware.dumpedAt).toLocaleString()}
                    </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={
                progress.stage === "dumping" || progress.stage === "analyzing"
              }
              ...
              <>
                <button
                  ...
                  disabled={
                    ...
                    (archInfo !== null && !archInfo.supported)
                  }
                  className="px-4 py-2 bg-green-600 hover:bg-green-700 disabled:bg-gray-700 disabled:cursor-not-allowed rounded text-sm font-medium transition-colors"
                  title={
                    archInfo && !archInfo.supported
                      ? `${archInfo.architecture} not supported`
                      : "Re-run WASM analysis (preserves comments and renames)"
                  }
                >
              ...
              <div className="space-y-2 text-sm font-mono">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-green-400">
                    {dump.chipInfo?.voltage
                      ? `${dump.chipInfo.voltage.toFixed(2)}V`
                      : "N/A"}
                  </span>
                </div>
                ...
                {archInfo && (
                  <>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span
                        className={
                          archInfo.supported ? "text-green-400" : "text-red-400"
                        }
                      >
                        {archInfo.supported
                          ? "✅ Supported"
                ...
                <div className="flex justify-between">
                  ...
                  <span>
                    {dump.size.toLocaleString()} bytes (
                    {(dump.size / 1024).toFixed(1)} KB)
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono text-green-400">
                      {analysisResults.xrefs.length}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">Total Instructions:</span>
                    <span className="font-mono text-green-400">
                      {analysisResults.total_instructions}
                    </span>
                  </div>
                </div>
                ...
                {analysisResults.xrefs && analysisResults.xrefs.length > 0 && (
                  <div className="border-t border-gray-700 pt-3">
                    <h4 className="text-xs font-semibold text-gray-400 mb-2">
                      Cross-References (showing first 20)
                    </h4>
                    <div className="max-h-64 overflow-y-auto">
                      <table className="w-full text-xs font-mono">
                        ...
                        <tbody>
                          {analysisResults.xrefs
                            .slice(0, 20)
                            .map((xref: XrefResult, idx: number) => (
                              <tr
                                key={idx}
                      ...
                      {analysisResults.xrefs.length > 20 && (
                        <div className="text-center text-xs text-gray-500 py-2">
                          ... and {analysisResults.xrefs.length - 20} more
                          cross-references
                        </div>
                      )}
                    </div>
