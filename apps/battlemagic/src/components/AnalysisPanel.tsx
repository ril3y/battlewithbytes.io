...
import {
  loadWasmAnalyzer,
  type WasmAnalyzerModule,
} from "../lib/loadWasmAnalyzer";
...
export default function AnalysisPanel({
  ...
  const { setAnalyzer, setAnalysisStats, analysisStats, clearAnalysis } =
    useXref();
  ...
  const [detectionResult, setDetectionResult] =
    useState<MemoryDetectionResult | null>(null);
    ...
    const loadWasm = async () => {
      ...
      try {
        ...
        if (mounted) {
          ...
          onOutput?.(
            `[Analysis Error] Failed to load WASM module: ${errorMsg}`,
          );
  ...
  const handleAutoDetect = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        onOutput?.(
          `[Detection] MCU detected: ${result.mcu.name} (${result.mcu.family})`,
        );
        onOutput?.(
          `[Detection] Flash base: 0x${result.recommendedFlashBase.toString(16).toUpperCase()}`,
        );
  ...
  const handleAnalyzeBinary = useCallback(async () => {
    ...
    try {
      ...
      if (!manualConfig.force && !currentDetectionResult) {
        onOutput?.(
          "[Analysis] No configuration detected - running auto-detection first...",
        );
        try {
          const detector = new MemoryDetector(gdbClient);
          const result = await detector.detect();
      ...
      onOutput?.(
        `[Analysis] Reading ${length} (0x${length.toString(16)}) bytes from address 0x${baseAddr.toString(16).toUpperCase()}...`,
      );
      ...
      if (!memory || memory.length === 0) {
        throw new Error(
          "Failed to read memory from target - GDB returned no data",
        );
      }

      onOutput?.(
        `[Analysis] Successfully read ${memory.length} bytes from 0x${baseAddr.toString(16).toUpperCase()}`,
      );
      ...
      if (isAllFF) {
        throw new Error(
          "Memory appears to be erased (all 0xFF) - flash may be blank or protected",
        );
      }
      if (isAllZero) {
        throw new Error(
          "Memory appears to be blank (all 0x00) - device may not be programmed",
        );
      }
      ...
      if (baseAddr === 0x0 && memory.length >= 8) {
        ...
        const initialSP = new DataView(
          memory.buffer,
          memory.byteOffset,
          4,
        ).getUint32(0, true);
        const resetVector = new DataView(
          memory.buffer,
          memory.byteOffset + 4,
          4,
        ).getUint32(0, true);
        ...
        onOutput?.(
          `[Analysis] Vector Table: Initial SP=0x${initialSP.toString(16).toUpperCase()}, Reset=0x${resetVector.toString(16).toUpperCase()}`,
        );
        ...
        if (
          resetAddr >= baseAddr &&
          resetAddr < baseAddr + memory.length - 16
        ) {
          const offset = resetAddr - baseAddr;
          disasmStartAddr = resetAddr;
          disasmMemory = memory.slice(offset);
          onOutput?.(
            `[Analysis] Starting disassembly at reset vector: 0x${resetAddr.toString(16).toUpperCase()}`,
          );
        } else {
          // Reset vector is outside our read range - need to read from that address
          onOutput?.(
            `[Analysis] Reset vector 0x${resetAddr.toString(16).toUpperCase()} is outside initial read range`,
          );
          onOutput?.(
            `[Analysis] Reading from reset vector at 0x${resetAddr.toString(16).toUpperCase()}...`,
          );
          ...
          try {
            ...
            onOutput?.(
              `[Analysis] Successfully read ${resetMemory.length} bytes from reset vector`,
            );
          } catch (error) {
            const errorMsg =
              error instanceof Error ? error.message : String(error);
            onOutput?.(
              `[Analysis] Failed to read from reset vector: ${errorMsg}`,
            );
            ...
            if (resetAddr < baseAddr + memory.length) {
              ...
              disasmMemory = memory.slice(offset);
              onOutput?.(
                `[Analysis] Using offset ${offset} bytes from initial read`,
              );
            } else {
              // Last resort: skip standard 256-byte vector table
              onOutput?.(
                `[Analysis] Reset vector too far, skipping 256-byte vector table`,
              );
      ...
      // Debug logging
      onOutput?.(
        `[Analysis] Disassembler config: addr=0x${disasmStartAddr.toString(16)}, length=${disasmMemory.length}, thumb=${useThumb}`,
      );
      ...
      const instructions = await disassembler.disassemble(
        disasmMemory,
        disasmStartAddr,
        useThumb,
      );
      ...
      instructions.forEach((inst) => {
        const xrefs = analyzer.get_xrefs_from(inst.address) as Xref[];
        xrefs.forEach((xref) => {
      ...
      onOutput?.(
        `[Analysis] Found ${results.totalInstructions} instructions, ${results.functionsDetected} functions, ${results.xrefsBuilt} cross-references`,
      );
              ...
              {detectionResult && (
                <div className="space-y-2 mb-3">
                  ...
                  <div className="flex items-center justify-between py-1 border-b border-gray-800">
                    ...
                    <span className="text-xs text-cyan-400 font-mono">
                      0x
                      {detectionResult.recommendedFlashBase
                        .toString(16)
                        .toUpperCase()}
                    </span>
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setManualConfig({
                          ...manualConfig,
                          readSize: parseInt(e.target.value),
                        })
                      }
                      className="w-full px-2 py-1 text-xs bg-gray-900 border border-gray-700 rounded font-mono text-cyan-400"
                    >
                  ...
                  <div className="space-y-1 font-mono text-xs">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-yellow-400">
                        0x
                        {analysisStats.addressRange.start
                          .toString(16)
                          .toUpperCase()}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-yellow-400">
                        0x
                        {analysisStats.addressRange.end
                          .toString(16)
                          .toUpperCase()}
                      </span>
                    </div>
