...
export default function BreakpointsManager({
  ...
  const [breakpointType, setBreakpointType] = useState<"hardware" | "software">(
    "software",
  );
  ...
  const generateId = () =>
    `bp_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  const handleAddBreakpoint = useCallback(async () => {
    ...
    try {
      ...
      if (newAddress) {
        ...
        const addressNum =
          addressOrSymbol.startsWith("0x") || addressOrSymbol.startsWith("0X")
      ...
      await gdbClient.insertBreakpoint(
        gdbAddress,
      ...
      onOutput?.(
        `[Breakpoint] Added ${breakpointType} breakpoint at ${displayAddress}`,
      );
  ...
  const handleRemoveBreakpoint = useCallback(
    async (bp: Breakpoint) => {
      ...
      try {
        ...
        const errorMsg = error instanceof Error ? error.message : String(error);
        onOutput?.(
          `[Error] Failed to remove breakpoint at ${bp.address}: ${errorMsg}`,
        );
      }
    },
  ...
  const handleToggleBreakpoint = useCallback(
    async (bp: Breakpoint) => {
      ...
      try {
        ...
        onOutput?.(
          `[Breakpoint] ${bp.enabled ? "Disabled" : "Enabled"} breakpoint at ${bp.address}`,
        );
      } catch (error) {
        const errorMsg = error instanceof Error ? error.message : String(error);
        onOutput?.(
          `[Error] Failed to toggle breakpoint at ${bp.address}: ${errorMsg}`,
        );
      }
    },
    [gdbClient, isConnected, onOutput, setBreakpoints],
  ...
  const handleClearAll = useCallback(async () => {
    ...
    if (failCount === 0) {
      onOutput?.(
        `[Breakpoint] Cleared all breakpoints (${successCount} removed)`,
      );
    } else {
      onOutput?.(
        `[Breakpoint] Cleared breakpoints: ${successCount} removed, ${failCount} failed`,
      );
            ...
            if (bp.enabled) {
              try {
                ...
                const errorMsg =
                  error instanceof Error ? error.message : String(error);
                onOutput?.(
                  `[Warning] Failed to set breakpoint at ${bp.address}: ${errorMsg}`,
                );
                bp.enabled = false;
              }
            }
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setBreakpointType(e.target.value as "hardware" | "software")
                }
