...
export const ControlFlowGraphView: React.FC<ControlFlowGraphViewProps> = ({
  ...
  const [blockXrefInfo, setBlockXrefInfo] = useState<
    Map<string, BlockXrefInfo>
  >(new Map());

  // Compute xref information for a block
  const computeBlockXrefInfo = useCallback(
    (block: BasicBlock): BlockXrefInfo => {
      if (!isAnalyzed()) {
        return {
          incomingCalls: 0,
          outgoingCalls: 0,
      ...
      for (const inst of block.instructions) {
        ...
        for (const xref of xrefsTo) {
          if (xref.xref_type === "Call") incomingCalls++;
            ...
            xref.xref_type === "Branch" ||
            xref.xref_type === "ConditionalBranch"
          )
            incomingBranches++;
        }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (containerRef.current && computedLayout) {
        ...
        // Center the graph
        const offsetX =
          (canvasWidth - bounds.width * fitScale) / 2 - bounds.minX * fitScale;
        const offsetY =
          (canvasHeight - bounds.height * fitScale) / 2 -
          bounds.minY * fitScale;

        setTransform({ offsetX, offsetY, scale: fitScale });
        console.log("[CFG] Auto-fit:", {
          scale: fitScale,
          offsetX,
          offsetY,
          bounds,
  ...
  useEffect(() => {
    ...
    for (const [blockId, block] of cfg.blocks) {
      if (
        selectedAddress >= block.startAddress &&
        selectedAddress <= block.endAddress
      ) {
  ...
  const getEdgeStyleForXref = useCallback(
    (
      edge: EdgeLayout,
    ): { color: string; dash: number[]; width: number; label?: string } => {
      if (!isAnalyzed() || !useXrefLayout || !cfg) {
        return {
          color: edge.color,
          dash: edge.isBackEdge ? [5, 5] : [],
          width: 3,
        };
      }
      ...
      for (const inst of sourceBlock.instructions) {
        ...
        for (const xref of xrefs) {
          ...
          if (
            xref.to_addr >= targetBlock.startAddress &&
            xref.to_addr <= targetBlock.endAddress
          ) {
  ...
  const drawEdge = useCallback(
    (ctx: CanvasRenderingContext2D, edge: EdgeLayout) => {
  ...
  const getBlockAtPosition = useCallback(
    ...
    >
      ...
      {tooltipInfo && (
        <div
          ...
          style={{
            ...
            boxShadow:
              "0 0 10px rgba(0, 136, 255, 0.4), 0 0 20px rgba(0, 255, 157, 0.2)",
            border: "1px solid #0088ff",
            borderLeft: "3px solid #00ff9d",
            backgroundImage:
              "linear-gradient(135deg, rgba(0, 136, 255, 0.05) 25%, transparent 25%, transparent 50%, rgba(0, 136, 255, 0.05) 50%, rgba(0, 136, 255, 0.05) 75%, transparent 75%, transparent)",
            backgroundSize: "20px 20px",
          }}
        >
          <div className="font-bold text-blue-400 mb-2">
            {tooltipInfo.block.id}
          </div>
          <div className="text-gray-400 mb-1">
            Type:{" "}
            <span className="text-amber-400">
              {tooltipInfo.block.type.toUpperCase()}
            </span>
          </div>
          <div className="text-gray-400 mb-1">
            Range:{" "}
            <span className="text-green-400">
              0x{tooltipInfo.block.startAddress.toString(16).toUpperCase()}
            </span>
            {" - "}
            <span className="text-green-400">
              0x{tooltipInfo.block.endAddress.toString(16).toUpperCase()}
            </span>
          </div>
          <div className="text-gray-400 mb-2">
            Instructions:{" "}
            <span className="text-cyan-400">
              {tooltipInfo.block.instructions.length}
            </span>
          </div>
          ...
          <div
            className="absolute -bottom-1 -right-1 h-2 w-2 bg-[#00ff9d]"
      ...
      {isAnalyzed() ? (
        ...
        <div className="absolute top-2 left-1/2 transform -translate-x-1/2 bg-yellow-900/90 text-yellow-200 text-xs px-4 py-2 rounded border border-yellow-600 font-mono">
          No analysis data - run &apos;Tools → Binary Analysis&apos; for
          enhanced features
        </div>
      )}
      ...
      <div className="absolute bottom-2 left-2 bg-slate-800/90 text-slate-200 text-xs px-3 py-2 rounded border border-slate-600 font-mono max-w-xs">
        ...
        {isAnalyzed() && useXrefLayout && (
          <>
            <div className="font-bold mb-2 mt-3 pt-2 border-t border-slate-600">
              Edge Types:
            </div>
          ...
          <button
            onClick={() => {
              ...
              if (block) {
                ...
                if (xrefInfo) {
                  console.log(
                    `[CFG] Callers to ${contextMenu.blockId}:`,
                    xrefInfo.incomingCalls,
                  );
                  ...
                  for (const inst of block.instructions) {
                    ...
                    if (xrefs.length > 0) {
                      console.log(
                        `  0x${inst.address.toString(16)}: ${xrefs.length} xrefs`,
                      );
                      xrefs.forEach((xref) =>
                        console.log(
                          `    from 0x${xref.from_addr.toString(16)} (${xref.xref_type})`,
                        ),
                      );
                    }
                  }
          ...
          <button
            onClick={() => {
              ...
              if (block) {
                ...
                if (xrefInfo) {
                  console.log(
                    `[CFG] Callees from ${contextMenu.blockId}:`,
                    xrefInfo.outgoingCalls,
                  );
                  ...
                  for (const inst of block.instructions) {
                    ...
                    if (xrefs.length > 0) {
                      ...
                      xrefs.forEach((xref) =>
                        console.log(
                          `    to 0x${xref.to_addr.toString(16)} (${xref.xref_type})`,
                        ),
                      );
                    }
                  }
              ...
              if (block && layout) {
                ...
                if (blockLayout) {
                  ...
                  if (canvas) {
                    ...
                    const offsetX =
                      canvas.width / 2 - centerX * transform.scale;
                    const offsetY =
                      canvas.height / 2 - centerY * transform.scale;
