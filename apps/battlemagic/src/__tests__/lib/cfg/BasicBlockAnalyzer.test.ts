...
describe("BasicBlockAnalyzer", () => {
  ...
  describe("Conditional Branches", () => {
    it("should split blocks on conditional branch", () => {
      ...
      expect(
        entryBlock.instructions[entryBlock.instructions.length - 1].mnemonic,
      ).toBe("beq");

      // Entry block with conditional branch can be either ENTRY or CONDITIONAL
      expect([BlockType.ENTRY, BlockType.CONDITIONAL]).toContain(
        entryBlock.type,
      );
    });
    ...
    it("should create edges for conditional branches", () => {
      ...
      const hasConditionalEdge = entryBlock.edges.some(
        (e) =>
          e.type === EdgeType.CONDITIONAL_TRUE ||
          e.type === EdgeType.CONDITIONAL_FALSE,
      );
  ...
  describe("Return Blocks", () => {
    it("should identify return block with bx lr", () => {
      ...
      const lastInst =
        blocks[0].instructions[blocks[0].instructions.length - 1];
  ...
  describe("Unconditional Branches", () => {
    it("should handle unconditional branch", () => {
      ...
      const unconditionalEdge = entryBlock.edges.find(
        (e) => e.type === EdgeType.UNCONDITIONAL,
      );
  ...
  describe("Edge Conditions", () => {
    it("should extract condition for beq", () => {
      ...
      const trueEdge = entryBlock.edges.find(
        (e) => e.type === EdgeType.CONDITIONAL_TRUE,
      );
      const falseEdge = entryBlock.edges.find(
        (e) => e.type === EdgeType.CONDITIONAL_FALSE,
      );
