  ...
  const wasiBindings = {
    ...
    fd_write: (fd: number, iovs: number, iovsLen: number, nwritten: number) => {
      ...
      if (fd === 1 || fd === 2) {
        ...
        const newSize = Math.max(
          existingData.length,
          fileInfo.position + totalWritten,
        );
    ...
    fd_prestat_dir_name: (fd: number, path_ptr: number, path_len: number) => {
      ...
      const bytes = new Uint8Array(
        memory.buffer,
        path_ptr,
        Math.min(path_len, encoded.length),
      );
    ...
    path_open: (
      ...
      console.log(
        `[VFS] path_open CALLED: fd=${fd}, oflags=0x${oflags.toString(16)}`,
      );

      const pathStr = readString(path_ptr, path_len);
      const dirPath = preopens.get(fd) || "/";
      let fullPath = pathStr.startsWith("/")
        ? pathStr
        : dirPath + "/" + pathStr;
      ...
      console.log(
        `[VFS] path_open: ${fullPath}, exists: ${pathExists}, isDir: ${isDir}, isFile: ${isFile}, O_DIRECTORY=${!!(oflags & O_DIRECTORY)}`,
      );
      ...
      if (!pathExists) {
        ...
        if (oflags & O_DIRECTORY && isFile) {
          console.log(
            `[VFS] path_open: O_DIRECTORY set but ${fullPath} is a file, returning ENOTDIR`,
          );
      ...
      console.log(
        `[VFS] path_open: SUCCESS - opened ${fullPath} as fd ${fd_num}`,
      );
      return 0;
    },
    ...
    path_filestat_get: (
      ...
      console.log(
        `[VFS] path_filestat_get: fd=${fd}, path="${pathStr}", fullPath="${fullPath}", exists: ${pathExists}, isDir: ${isDir}, isFile: ${isFile}`,
      );
      ...
      view.setBigUint64(
        buf_ptr + 32,
        BigInt(fileData ? fileData.length : 0),
        true,
      ); // size
    ...
    path_rename: (
      ...
      let oldFullPath = oldPathStr.startsWith("/")
        ? oldPathStr
        : oldDirPath + "/" + oldPathStr;
      let newFullPath = newPathStr.startsWith("/")
        ? newPathStr
        : newDirPath + "/" + newPathStr;
      ...
      console.log(
        `[VFS] path_rename: SUCCESS - renamed ${oldFullPath} to ${newFullPath} (${data.length} bytes)`,
      );
      return 0;
    },
  ...
  const importsObj = {
    ...
    env: {
      ...
      __syscall_openat: (dirfd: number, path_ptr: number, flags: number) => {
        ...
        console.log(
          `[VFS] __syscall_openat: dirfd=${dirfd}, path="${pathStr}", flags=${flags}`,
        );
        ...
        if (dirfd === AT_FDCWD || dirfd < 0) {
          ...
          console.log(
            `[VFS] __syscall_openat: Converted special dirfd ${dirfd} to root preopen (3)`,
          );
        }
        ...
        if (result !== 0) {
          console.log(
            `[VFS] __syscall_openat: path_open failed with error ${result} for "${pathStr}"`,
          );
        ...
        console.log(
          `[VFS] __syscall_openat: Successfully opened "${pathStr}" as fd ${fd}`,
        );
        return fd;
      },
      ...
      __syscall_newfstatat: (
        ...
        console.log(
          `[VFS] __syscall_newfstatat: dirfd=${dirfd}, path="${pathStr}", flags=${flags}`,
        );
        ...
        if (dirfd === AT_FDCWD || dirfd < 0) {
          actualDirFd = 3; // Use root preopen directory
          console.log(
            `[VFS] __syscall_newfstatat: Converted special dirfd ${dirfd} to root preopen (3)`,
          );
        }

        // Use WASI path_filestat_get
        const result = wasi.path_filestat_get(
          actualDirFd,
          flags,
          path_ptr,
          pathLen,
        ...
        console.log(
          `[VFS] __syscall_newfstatat: path_filestat_get returned ${result} for "${pathStr}"`,
        );
        return result === 0 ? 0 : -result;
      },
      ...
      __syscall_unlinkat: (dirfd: number, path_ptr: number, flags: number) => {
        ...
        console.log(
          `[VFS] __syscall_unlinkat: ${deleted ? "deleted" : "not found"} ${fullPath}`,
        );
      ...
      __syscall_renameat: (
        ...
        let oldLen = 0;
        while (memoryView[oldpath_ptr + oldLen] !== 0 && oldLen < 4096)
          oldLen++;
        const oldPath = new TextDecoder().decode(
          new Uint8Array(memory.buffer, oldpath_ptr, oldLen),
        );

        // Read new path
        let newLen = 0;
        while (memoryView[newpath_ptr + newLen] !== 0 && newLen < 4096)
          newLen++;
        const newPath = new TextDecoder().decode(
          new Uint8Array(memory.buffer, newpath_ptr, newLen),
        );
