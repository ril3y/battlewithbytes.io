...
export class ArduinoIndexParser {
  ...
  private static parsePlatform(
    ...
    if (boards.length > 0 && boards[0].build?.mcu) {
      cpuArchitecture = refineMcuArchitecture(boards[0].build.mcu, archMapping);
    }
  ...
  static filterByArchitecture(
    ...
    return platforms.filter((p) => archSet.has(p.architecture.toLowerCase()));
  }
 ...
 */
export function parseArduinoBuildFlags(board: ArduinoBoard): {
