...
import { IPacketObserver, BaseDecoder } from '../lib/swo/BaseDecoder';
import {
  SwoPacket,
  PacketEvent,
  DecoderError,
 ...
 */
export default function SwoViewer({ gdbClient, isConnected, onOutput }: SwoViewerProps) {
  // State
  const [isCapturing, setIsCapturing] = useState(false);
  const [entries, setEntries] = useState<TraceEntry[]>([]);
  const [isPaused, setIsPaused] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [enabledPorts, setEnabledPorts] = useState<Set<number>>(new Set(Array.from({ length: 32 }, (_, i) => i)));
  const [showTimestamps, setShowTimestamps] = useState(true);
  const [showRawData, setShowRawData] = useState(false);
  const [baudRate, setBaudRate] = useState(2000000); // 2MHz default
   ...
   */
  const createTraceEntry = useCallback((event: PacketEvent): TraceEntry | null => {
    ...
    const entry: TraceEntry = {
      ...
      type: packet.type,
      text: '',
      rawHex: Array.from(packet.raw).map(b => b.toString(16).padStart(2, '0')).join(' '),
    ...
    switch (packet.type) {
      case PacketType.INSTRUMENTATION: {
        ...
        entry.text = inst.text || `[Port ${inst.port}] ${formatHexData(inst.data)}`;
   ...
   */
  const filterEntries = useCallback((entries: TraceEntry[]): TraceEntry[] => {
    ...
    <div className="h-full flex flex-col bg-gray-950">
      ...
      <div className="bg-gray-900 border-b border-gray-700 p-3">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-lg font-semibold text-green-400">SWO Trace Viewer</h3>
        ...
        <div className="flex items-center gap-4 text-sm">
          ...
          {statistics && (
            <div className="ml-auto text-xs text-gray-500">
              Packets: {statistics.packetsReceived} |
              Errors: {statistics.syncErrors} |
              Overflows: {statistics.overflowEvents}
            </div>
          )}
        </div>
          ...
          <div className="flex flex-wrap gap-1">
            ...
            <button
              onClick={() => {
                ...
                setEnabledPorts(new Set(enabledPorts.size === 32 ? [] : allPorts));
              }}
      ...
      >
        {filteredEntries.length === 0 ? (
          <div className="text-gray-600 text-center py-8">
            {isCapturing ? 'Waiting for SWO data...' : 'No trace data. Start capture to begin.'}
          </div>
        ) : (
          <div className="space-y-0.5">
            {filteredEntries.map(entry => (
              <div key={entry.id} className="flex items-start gap-2 hover:bg-gray-900 px-1 py-0.5">
                {showTimestamps && (
